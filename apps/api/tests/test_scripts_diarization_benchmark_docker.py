...
def test_benchmark_docker_script_does_not_embed_hf_token_values_in_cli_args():
    ...
    assert 'compose_run_env_args=(-e "HF_TOKEN=${hf_token}" -e "HUGGINGFACE_TOKEN=${hf_token}")' not in text, (
        "HF token values must not be passed directly in docker CLI arguments"
    )
    assert "compose_run_env_args=(-e HF_TOKEN -e HUGGINGFACE_TOKEN)" in text, (
