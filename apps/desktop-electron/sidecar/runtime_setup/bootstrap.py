...
RunStep = Callable[[Sequence[str], Optional[Dict[str, str]]], int]
...
def render_pth(
    env_dir: Path | str,
    sidecar_src: Optional[Path | str] = None,
    *,
...
def find_pth_file(embed_dir: Path | str) -> Optional[Path]:
    ...
    for index, step in enumerate(steps, start=1):
        ...
        if code != 0:
            raise BootstrapError(
                f"setup step {index} failed (exit {code}): {' '.join(step['argv'])}"
            )
...
def ensure_assets(names: Sequence[str], root: Path, *, manager: Optional[Any] = None) -> None:
...
def extract_tool_archives(
    root: Path,
    *,
    archives: Optional[Sequence[Any]] = None,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="print the planned steps and exit"
    )
    return parser
