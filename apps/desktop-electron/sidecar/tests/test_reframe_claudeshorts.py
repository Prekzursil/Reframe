...
def test_smooth_centers_damps_jitter():
    ...
    raw_jump = max(abs(b - a) for a, b in zip(raw, raw[1:]))
    out_jump = max(abs(b - a) for a, b in zip(out, out[1:]))
...
def test_build_reframe_argv_one_pass_shape(fake_bins):
    ...
    argv = cs.build_reframe_argv("C:\\in\\a clip.mp4", "C:\\out\\b clip.mp4", crop,
                                 None, "9:16", fake_bins)
...
def test_detect_subject_centers_center_backend_short_circuits():
    ...
    assert cs.detect_subject_centers("/in.mp4", [0.5], frame_runner=boom,
                                     backend="center") == []
...
def test_resolve_explicit_claudeshorts_never_probes():
    name, notice = reframe.resolve_engine_name(
        "claudeshorts", {}, probe_runner=_never_probe
    )
...
def test_resolve_verthor_when_available(requested, verthor_script):
    name, notice = reframe.resolve_engine_name(
        requested, verthor_script, probe_runner=_wsl_probe(returncode=0)
    )
    assert name == "verthor"
    assert notice is None
...
def test_resolve_falls_back_when_wsl_probe_fails(requested, verthor_script):
    name, notice = reframe.resolve_engine_name(
        requested, verthor_script, probe_runner=_wsl_probe(returncode=1)
    )
    assert name == "claudeshorts"
...
def test_get_engine_returns_constructed_instances(verthor_script):
    eng, notice = reframe.get_engine(
        "auto", verthor_script, probe_runner=_wsl_probe(returncode=0)
    )
    assert isinstance(eng, ReframeEngine)
    assert notice is None

    eng, notice = reframe.get_engine(
        "auto", verthor_script, probe_runner=_wsl_probe(returncode=1)
    )
