...
class FakeLoader:
    ...
    def __init__(self, model: FakeModel, *, fail_on: Optional[set] = None):
        self._model = model
...
def test_progress_is_monotonic_and_ends_at_100():
    ...
    transcribe.transcribe_file(
        "/v.mp4", loader=loader, on_progress=lambda p, m: events.append((p, m))
    )
...
def test_handler_returns_jobid_immediately_and_streams_done():
    ...
    loader = FakeLoader(_two_segment_model())
    handler = transcribe.make_transcribe_handler(
        lambda vid: "/media/v.mp4" if vid == "v1" else None, loader=loader
    )
...
def test_handler_marks_transcribed_hook_called():
    ...
    marked: List[str] = []
    handler = transcribe.make_transcribe_handler(
        lambda vid: "/m/v.mp4", loader=loader, on_transcribed=marked.append
    )
    out = handler({"videoId": "v1"}, ctx)
    registry.get(out["jobId"]).wait(timeout=5.0)
    assert marked == ["v1"]
...
def test_handler_transcribed_hook_failure_does_not_fail_job():
    ...
    handler = transcribe.make_transcribe_handler(
        lambda vid: "/m/v.mp4", loader=loader, on_transcribed=boom
    )
    out = handler({"videoId": "v1"}, ctx)
    job = registry.get(out["jobId"])
