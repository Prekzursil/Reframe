...
class _FakePopen:
    ...
    def __init__(
        ...
        self.job_snapshot: Optional[Dict[str, Any]] = None
        self.proc: Optional[_FakeProc] = None
...
def dev_root(tmp_path: Path) -> Path:
    electron = (
        tmp_path / "app" / "node_modules" / "electron" / "dist" / f"electron{cr._EXE}"
    )
...
def test_detect_chrome_headless_shell_settings_hit(
...
def test_detect_chrome_headless_shell_extracted_hit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def _exe_member() -> str:
    ...
    return (
        Path(cr.CHROME_HEADLESS_SHELL_EXE_REL)
        .relative_to(cr.CHROME_HEADLESS_SHELL_EXTRACT_DIR)
        .as_posix()
    )
...
def test_resolve_node_exe_env_missing_file_falls_through(
    tmp_path: Path, dev_root: Path
) -> None:
    settings_exe = tmp_path / "settings-node.exe"
    settings_exe.write_bytes(b"x")
    result = resolve_node_exe(
...
def test_resolve_render_js_chain(tmp_path: Path, dev_root: Path) -> None:
    ...
    assert (
        resolve_render_js({}, env={cr.ENV_RENDER_JS: str(override)}, dev_root=dev_root)
        == str(override)
    )
    assert (
        resolve_render_js(
            {cr.SETTING_RENDER_JS: str(override)}, env={}, dev_root=dev_root
        )
        == str(override)
    )
...
def test_resolve_bundle_dir_chain(tmp_path: Path, dev_root: Path) -> None:
    ...
    override.mkdir()
    assert (
        resolve_bundle_dir({}, env={cr.ENV_BUNDLE_DIR: str(override)}, dev_root=dev_root)
        == str(override)
    )
    assert (
        resolve_bundle_dir(
            {cr.SETTING_BUNDLE_DIR: str(override)}, env={}, dev_root=dev_root
        )
        == str(override)
    )
...
def test_resolve_chromium_chain_and_soft_miss(tmp_path: Path) -> None:
    ...
    assert (
        resolve_chromium({}, env={cr.ENV_CHROME: str(env_exe)}, assets_root=assets_root)
        == str(env_exe)
    )
...
def test_build_job_shape_and_rebase() -> None:
    ...
    assert set(props.keys()) == {
        "videoSrc", "cues", "style", "width", "height", "durationInSeconds",
    }
...
def test_build_job_drops_cues_entirely_before_clip() -> None:
    ...
    job = build_job(
        "c.mp4", cues, "o.mp4", bundle_dir="b", source_start=120.0, total_sec=5.0
    )
...
def test_build_job_chromium_only_when_provided() -> None:
    ...
    with_chrome = build_job(
        "c.mp4", [], "o.mp4", bundle_dir="b", chromium_executable="C:/chs/chs.exe"
    )
...
def test_build_job_carries_emphasis_and_emoji() -> None:
    cues = [
        {
            "index": 1, "start": 0.0, "end": 1.0, "text": "FREE money",
            "emphasis": [
...
def test_build_job_drops_malformed_emphasis_spans() -> None:
    ...
    job = build_job("c.mp4", cues, "o.mp4", bundle_dir="b", total_sec=1.0)
    assert job["inputProps"]["cues"][0]["emphasis"] == [
        {"start": 0, "end": 2, "kind": "long"}
    ]
...
def test_run_render_cancel_terminates_process() -> None:
    ...
    code, ok_path, _tail = run_render(
        ["exe", "r.js", "j.json"], should_cancel=should_cancel, popen=popen
    )
...
def test_engine_render_includes_chromium_when_asset_present(
    dev_root: Path, tmp_path: Path
) -> None:
    assets_root = tmp_path / "assets"
    _make_chrome_zip(assets_root / cr.CHROME_HEADLESS_SHELL_ZIP_DEST)
    popen = _FakePopen(stdout_lines=["RENDER_OK o.mp4\n"], code=0)
    engine = RemotionCaptionEngine(
        settings={}, popen=popen, env={}, dev_root=dev_root, assets_root=assets_root
    )
...
def test_engine_render_failure_raises_with_stderr_tail(
    dev_root: Path, tmp_path: Path
) -> None:
    popen = _FakePopen(
        stdout_lines=[],
...
def test_engine_render_exit_zero_without_render_ok_is_a_failure(
    dev_root: Path, tmp_path: Path
) -> None:
...
def test_engine_render_cleans_job_file_on_failure(
    dev_root: Path, tmp_path: Path
) -> None:
...
def test_is_transient_compositor_failure_matches_known_signatures() -> None:
    ...
    assert is_transient_compositor_failure(
...
def test_engine_render_retries_once_then_succeeds_on_transient(
    dev_root: Path, tmp_path: Path
) -> None:
...
def test_engine_render_raises_after_retries_when_transient_persists(
    dev_root: Path, tmp_path: Path
) -> None:
...
def test_engine_render_does_not_retry_non_transient_failure(
    dev_root: Path, tmp_path: Path
) -> None:
...
def test_engine_render_does_not_retry_on_cancel(
    dev_root: Path, tmp_path: Path
) -> None:
