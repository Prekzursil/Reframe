...
def cand(hook: str = "hook", *, factors=None, language=None) -> Dict[str, Any]:
    c: Dict[str, Any] = {
        "rank": 1, "start": 0.0, "end": 30.0, "durationSec": 30.0,
        "hook": hook, "why": "w", "score": 80, "sourceStart": 0.0,
    }
...
def factors(avg: float) -> Dict[str, int]:
    v = int(avg)
    return {"hookStrength": v, "emotionalFlow": v,
            "perceivedValue": v, "shareability": v}
...
def test_register_wires_record_and_stats(store):
    ...
    fb.register(
        register_fn=lambda name, h: registered.__setitem__(name, h), store=store
    )
...
def test_exemplar_block_formats_top5_each_side(store):
    ...
    approved_lines = [l for l in block.splitlines() if l.startswith("+ ")]
    discarded_lines = [l for l in block.splitlines() if l.startswith("- ")]
