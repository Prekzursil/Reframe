...
def test_render_cue_text_skips_overlapping_and_out_of_range_spans():
    c = {
        "index": 1, "start": 0.0, "end": 1.0, "text": "abcdef",
        "emphasis": [
...
def test_build_ass_emits_emphasis_and_emoji_in_dialogue():
    doc = build_ass(
        [
            {
                "index": 1, "start": 0.0, "end": 2.0, "text": "huge WIN",
...
def test_render_rebases_inside_written_ass(fake_ffmpeg, monkeypatch):
    ...
    def runner(argv, total_sec=0.0, on_progress=None, should_cancel=None):
        ...
        with open(written["path"], "r", encoding="utf-8") as fh:
...
def test_render_uses_width_height_in_ass(fake_ffmpeg, monkeypatch):
    ...
    def runner(argv, total_sec=0.0, on_progress=None, should_cancel=None):
        with open(captured["path"], "r", encoding="utf-8") as fh:
...
def test_build_ass_emits_top_anchored_title_style_and_event():
    ...
    style_line = next(l for l in doc.splitlines() if l.startswith("Style: HookTitle,"))
    ...
    title_event = next(
        l for l in doc.splitlines() if l.startswith("Dialogue:") and "HookTitle" in l
    )
...
def test_build_ass_title_event_precedes_body_captions():
    doc = build_ass(
        [cue(1, 0.0, 2.0, "bodytext")], hook_title="HOOK", total_sec=10.0
    )
    lines = [l for l in doc.splitlines() if l.startswith("Dialogue:")]
...
def test_build_ass_title_duration_falls_back_to_last_cue():
    ...
    doc = build_ass(
        [cue(1, 100.0, 108.0, "x")], source_start=100.0, hook_title="H"
    )
    title_event = next(
        l for l in doc.splitlines() if l.startswith("Dialogue:") and "HookTitle" in l
    )
...
def test_render_threads_hook_title_into_written_ass(fake_ffmpeg, monkeypatch):
    ...
    eng = CaptionEngine(runner=runner)
    eng.render(
        "/in.mp4", [cue(1, 0.0, 1.0, "body")], "/out.mp4", hook_title="My Hook"
    )
