...
class TestManifest:
    ...
    @pytest.mark.parametrize(
        "bad",
        [
            dict(name="", kind="model", size_mb=1, dest="d", url="u"),
            dict(name="x", kind="weights", size_mb=1, dest="d", url="u"),
            dict(name="x", kind="model", size_mb=-1, dest="d", url="u"),
            dict(name="x", kind="model", size_mb=1, dest="d", url="u", installer="curl"),
            # download installer requires url + dest
            dict(name="x", kind="model", size_mb=1, dest="d"),
            dict(name="x", kind="model", size_mb=1, url="u"),
            # hf installer requires hf_repo
            dict(name="x", kind="model", size_mb=1, installer="hf"),
            # env installer requires dest + requirements
            dict(name="x", kind="env", size_mb=1, installer="env", dest="envs/x"),
        ],
    )
...
class TestEnvInstallArgv:
    ...
    def test_argvs_carry_no_loose_requirements(self, tmp_path):
        steps = build_env_install_argvs(
            "py", tmp_path / "gp.py", tmp_path / "e", ("numpy==2.1.0",)
        )
...
class TestDownload:
    def test_fresh_download_atomic_and_no_range(self, tmp_path):
        ...
        client = FakeClient(
            [FakeResponse(200, {"Content-Length": "11"}, chunks=body)]
        )
        ...
        assert fracs[-1] == 1.0
        assert all(b <= a for a, b in zip(fracs[1:], fracs))  # non-decreasing
    ...
    def test_server_ignoring_range_restarts_clean(self, tmp_path):
        ...
        client = FakeClient(
            [FakeResponse(200, {"Content-Length": "8"}, chunks=[b"fullbody"])]
        )
        mgr = make_manager(tmp_path, client=client)
        dest = mgr.resolve_dest(entry)
        dest.parent.mkdir(parents=True)
    ...
    def test_cancel_mid_download_keeps_part_for_resume(self, tmp_path):
        ...
        with pytest.raises(JobCancelled):
            mgr._install(
                entry, on_frac=lambda f, m="": None, should_cancel=lambda: flag["cancel"]
            )
        assert not dest.exists()
...
class TestInstalledAndList:
    ...
    def test_hf_cache_env_precedence(self, tmp_path):
        ...
        assert (
            hf_cache_dir({"HF_HOME": str(tmp_path / "home")})
            == tmp_path / "home" / "hub"
        )
...
class TestEnvInstaller:
    ...
    def test_get_pip_downloaded_when_missing(self, tmp_path):
        client = FakeClient(
            [FakeResponse(200, {"Content-Length": "9"}, chunks=[b"# get-pip"])]
        )
        calls: List[List[str]] = []
...
class TestEnsureJob:
    def test_ensure_job_progress_and_done_payload(self, tmp_path, registry, collected):
        ...
        client = FakeClient(
            [FakeResponse(200, {"Content-Length": "1100"}, chunks=[body_a, body_b])]
        )
        mgr = make_manager(tmp_path, client=client)
    ...
    def test_preflight_blocks_low_disk_via_error_payload(
        self, tmp_path, registry, collected
    ):
...
class TestRpcRegistration:
    def test_register_wires_exactly_the_assets_methods(self, tmp_path):
        ...
        mgr = assets_rpc.register(
            make_manager(tmp_path), register_fn=lambda n, h: registered.update({n: h})
        )
