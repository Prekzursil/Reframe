...
def make_clip(
    directory: Path, stem: str, *, meta: Optional[Dict[str, Any]] = None
) -> Path:
    ...
    if meta is not None:
        sh.metadata_path(clip).write_text(
...
def test_build_metadata_null_virality_stays_none():
    meta = sh.build_metadata(
        video_id="", source_title="", template="", virality_pct=None,
        duration_sec=0.0, hook="", created_at=1.0,
    )
...
def test_build_metadata_bad_virality_becomes_none():
    meta = sh.build_metadata(
        video_id="v", source_title="t", template="bold", virality_pct="NaN",
        duration_sec=1.0, hook="h", created_at=1.0,
    )
    assert meta["viralityPct"] is None
...
def test_write_then_read_metadata_round_trips(tmp_path):
    clip = tmp_path / "src-1.mp4"
    meta = sh.build_metadata(
        video_id="v1", source_title="T", template="neon", virality_pct=50,
        duration_sec=30.0, hook="hook", created_at=9.0,
    )
...
# --------------------------------------------------------------------------- #
SHORT_INFO_KEYS = {
    "id", "path", "videoId", "sourceTitle", "template", "viralityPct",
    "durationSec", "width", "height", "createdAt", "thumbnailPath", "hook",
}
...
def test_short_info_from_meta_carries_export_fields(tmp_path):
    meta = sh.build_metadata(
        video_id="v1", source_title="My Talk", template="hormozi",
        virality_pct=91, duration_sec=33.0, hook="A hook", created_at=100.0,
    )
...
def test_list_one_video_reads_metadata(exports_dir):
    ...
    meta = sh.build_metadata(
        video_id="v1", source_title="T", template="bold", virality_pct=80,
        duration_sec=20.0, hook="h", created_at=5.0,
    )
...
def test_list_uses_json_dims_without_probing(exports_dir):
    d = exports_dir / "shorts-v1"
    meta = sh.build_metadata(
        video_id="v1", source_title="", template="", virality_pct=None,
        duration_sec=20.0, hook="", created_at=5.0,
    )
    meta["width"] = 1080
    meta["height"] = 1920
    make_clip(d, "src-1", meta=meta)
    probe = RecordingRun()  # NOT a probe; use a sentinel that records calls
...
def test_list_all_videos_scans_every_shorts_dir(exports_dir):
    make_clip(
        exports_dir / "shorts-v1", "a",
        meta=sh.build_metadata(video_id="v1", source_title="", template="", virality_pct=None, duration_sec=1.0, hook="", created_at=1.0),
    )
    make_clip(
        exports_dir / "shorts-v2", "b",
        meta=sh.build_metadata(video_id="v2", source_title="", template="", virality_pct=None, duration_sec=1.0, hook="", created_at=2.0),
    )
    svc = service(exports_dir)
    out = svc.list({}, ctx())
...
def test_list_sorts_created_at_desc(exports_dir):
    d = exports_dir / "shorts-v1"
    make_clip(d, "old", meta=sh.build_metadata(video_id="v1", source_title="", template="", virality_pct=None, duration_sec=1.0, hook="", created_at=10.0))
    make_clip(d, "new", meta=sh.build_metadata(video_id="v1", source_title="", template="", virality_pct=None, duration_sec=1.0, hook="", created_at=99.0))
    svc = service(exports_dir)
    out = svc.list({"videoId": "v1"}, ctx())
...
def test_delete_removes_mp4_thumb_and_json(exports_dir):
    d = exports_dir / "shorts-v1"
    clip = make_clip(d, "src-1", meta=sh.build_metadata(
        video_id="v1", source_title="", template="", virality_pct=None,
        duration_sec=1.0, hook="", created_at=1.0))
    sh.thumbnail_path(clip).write_bytes(b"\xff\xd8jpeg")
    svc = service(exports_dir)
...
def test_reexport_returns_source_video_and_candidate(exports_dir):
    d = exports_dir / "shorts-v1"
    meta = sh.build_metadata(
        video_id="v1", source_title="T", template="neon", virality_pct=70,
        duration_sec=25.0, hook="re-open me", created_at=1.0,
    )
