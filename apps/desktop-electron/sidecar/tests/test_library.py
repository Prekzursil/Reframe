...
def test_add_returns_full_video_schema(lib: Library, fake_video: Path):
    ...
    assert set(v.keys()) == {
        "id", "path", "title", "addedAt", "durationSec", "hasTranscript",
    }
...
def _video(path: str, vid: str = "vid1") -> dict:
    return {
        "id": vid, "path": path, "title": "t",
        "addedAt": "2026-01-01T00:00:00Z", "durationSec": 10.0, "hasTranscript": False,
    }
