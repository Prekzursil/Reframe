...
class RecordingTranslator:
    def __init__(self, events: List[str], result: Optional[List[str]] = None):
        self.events = events
        self._result = result
...
class TestEncodeArgv:
    def test_aac_argv_shape(self):
        argv = d.build_encode_aac_argv(
            "C:/d ubs/dub.wav", "C:/d ubs/dub.m4a", {"ffmpegPath": "C:/ff/ffmpeg.exe"}
        )
...
def make_service(tmp_path, *, engines=None, voice_store=None, audio_tracks=None):
    ...
    return d.DubService(
        ...
        or VoiceStore(tmp_path / "voices", duration_probe=lambda p: 1.0),
...
class TestDubStartHandler:
    def test_unknown_engine_rejected(self, tmp_path, registry):
        ...
        with pytest.raises(RpcError, match="unknown engine"):
            service.dub_start(
                {"videoId": "v1", "trackId": "t1", "engine": "espeak"}, ctx
            )
    ...
    def test_chatterbox_requires_sample_id(self, tmp_path, registry):
        service = make_service(
            tmp_path, engines={"chatterbox": lambda: RecordingEngine([])}
        )
        ctx = RpcContext(emit_notification=lambda o: None, jobs=registry)
        with pytest.raises(RpcError, match="sampleId"):
            service.dub_start(
                {"videoId": "v1", "trackId": "t1", "engine": "chatterbox"}, ctx
            )
    ...
    def test_full_job_resolves_audio_track_and_wav_path(
        self, tmp_path, registry, collected
    ):
        audio_tracks = FakeAudioTracks()
        service = make_service(tmp_path, audio_tracks=audio_tracks)
        ctx = RpcContext(emit_notification=lambda o: None, jobs=registry)
        result = service.dub_start(
            {"videoId": "v1", "trackId": "t1", "engine": "fake", "voice": "af_x"}, ctx
        )
    ...
    def test_failures_surface_via_job_done_error_payload(
        self, tmp_path, registry, collected
    ):
        ...
        service = make_service(
            tmp_path, engines={"fake": lambda: ExplodingEngine([])}
        )
        ctx = RpcContext(emit_notification=lambda o: None, jobs=registry)
        result = service.dub_start(
            {"videoId": "v1", "trackId": "t1", "engine": "fake", "voice": "v"}, ctx
        )
        registry.join(timeout=10)
        done = [payload for kind, payload in collected if kind == "done"]
        assert len(done) == 1
