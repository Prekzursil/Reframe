...
class TestClassify:
    def test_h264_aac_in_mp4_is_playable(self):
        verdict, _ = mc.classify(
            probe_payload(MP4_FAMILY, [vstream("h264"), astream("aac")]), "C:/v/talk.mp4"
        )
        assert verdict == mc.VERDICT_PLAYABLE

    def test_h264_aac_in_mkv_needs_remux(self):
        verdict, reason = mc.classify(
            probe_payload(MKV_FAMILY, [vstream("h264"), astream("aac")]), "C:/v/talk.mkv"
        )
    ...
    def test_hevc_in_mkv_needs_proxy(self):
        verdict, reason = mc.classify(
            probe_payload(MKV_FAMILY, [vstream("hevc"), astream("aac")]), "C:/v/talk.mkv"
        )
    ...
    def test_hevc_in_mp4_still_needs_proxy(self):
        ...
        verdict, _ = mc.classify(
            probe_payload(MP4_FAMILY, [vstream("hevc"), astream("aac")]), "C:/v/talk.mp4"
        )
        assert verdict == mc.VERDICT_PROXY
    ...
    def test_legacy_video_codecs_need_proxy(self, codec: str):
        verdict, _ = mc.classify(
            probe_payload("avi", [vstream(codec), astream("mp3")]), "C:/v/old.avi"
        )
        assert verdict == mc.VERDICT_PROXY

    def test_unplayable_audio_forces_proxy(self):
        verdict, reason = mc.classify(
            probe_payload(MKV_FAMILY, [vstream("h264"), astream("dts")]), "C:/v/talk.mkv"
        )
        assert verdict == mc.VERDICT_PROXY
    ...
    def test_vp9_opus_webm_is_playable(self):
        verdict, _ = mc.classify(
            probe_payload(MKV_FAMILY, [vstream("vp9"), astream("opus")]), "C:/v/clip.webm"
        )
        assert verdict == mc.VERDICT_PLAYABLE
    ...
    def test_vp9_opus_in_mkv_extension_needs_remux(self):
        ...
        verdict, _ = mc.classify(
            probe_payload(MKV_FAMILY, [vstream("vp9"), astream("opus")]), "C:/v/clip.mkv"
        )
        assert verdict == mc.VERDICT_REMUX
    ...
    def test_unknown_codec_name_is_proxy(self):
        verdict, _ = mc.classify(
            probe_payload(MP4_FAMILY, [{"codec_type": "video"}]), "C:/v/odd.mp4"
        )
        assert verdict == mc.VERDICT_PROXY
...
class TestProbeMedia:
    ...
    def test_nonzero_exit_yields_empty(self, settings):
        out = mc.probe_media(
            "x.mp4", settings, runner=lambda *a, **k: FakeCompleted(1, "{}")
        )
        assert out == {}

    def test_garbage_stdout_yields_empty(self, settings):
        out = mc.probe_media(
            "x.mp4", settings, runner=lambda *a, **k: FakeCompleted(0, "not json")
        )
        assert out == {}

    def test_non_dict_json_yields_empty(self, settings):
        out = mc.probe_media(
            "x.mp4", settings, runner=lambda *a, **k: FakeCompleted(0, "[1,2]")
        )
        assert out == {}
...
class TestPlayable:
    def test_playable_verdict_for_h264_mp4(self, tmp_path, settings, direct_ctx):
        ...
        src.write_bytes(b"x")
        svc = make_service(
            tmp_path, settings, {"v1": str(src)},
            probe_payload(MP4_FAMILY, [vstream("h264"), astream("aac")]),
        )
        assert svc.playable({"videoId": "v1"}, direct_ctx) == {"playable": True}

    def test_remux_needed_reports_not_playable_with_reason(
        self, tmp_path, settings, direct_ctx
    ):
    ...
    def test_proxy_needed_reports_not_playable_with_reason(
        self, tmp_path, settings, direct_ctx
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_cached_proxy_short_circuits_to_proxy_path(
        self, tmp_path, settings, direct_ctx
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_source_mtime_change_invalidates_the_cache(
        self, tmp_path, settings, direct_ctx
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_unknown_video_id_raises_invalid_params(
        self, tmp_path, settings, direct_ctx
    ):
    ...
    def test_probe_crash_is_not_playable_never_a_server_error(
        self, tmp_path, settings, direct_ctx
    ):
        src = tmp_path / "talk.mp4"
        src.write_bytes(b"x")
...
class TestProxyStart:
    def test_hevc_source_builds_a_transcode_proxy_and_caches_it(
        self, tmp_path, settings, registry, collected
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
        run = RecordingRun()
        ...
        expected = mc.proxy_cache_path(
            tmp_path / "proxies", "v1", os.stat(src).st_mtime_ns
        )
    ...
    def test_h264_mkv_source_remuxes_with_stream_copy(
        self, tmp_path, settings, registry
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
        run = RecordingRun()
    ...
    def test_second_request_reuses_the_cache_without_re_encoding(
        self, tmp_path, settings, registry
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_playable_reports_the_built_proxy_afterwards(
        self, tmp_path, settings, registry, direct_ctx
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
        svc = make_service(
    ...
    def test_already_playable_source_returns_the_original_path(
        self, tmp_path, settings, registry
    ):
        src = tmp_path / "talk.mp4"
        src.write_bytes(b"x")
    ...
    def test_stale_cache_files_are_evicted_after_a_build(
        self, tmp_path, settings, registry
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_ffmpeg_failure_surfaces_via_the_job_done_error_payload(
        self, tmp_path, settings, registry, collected
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_cancellation_unwinds_and_cleans_the_partial(
        self, tmp_path, settings, registry, collected
    ):
        src = tmp_path / "talk.mkv"
        src.write_bytes(b"x")
    ...
    def test_unknown_video_fails_the_request_not_the_job(
        self, tmp_path, settings, registry
    ):
        svc = make_service(tmp_path, settings, {})
        ctx = RpcContext(emit_notification=lambda obj: None, jobs=registry)
