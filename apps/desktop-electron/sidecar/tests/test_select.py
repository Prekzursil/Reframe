...
def _long_transcript(n_segments: int = 600) -> Dict[str, Any]:
    ...
    segments = [
        {"start": float(i * 5), "end": float(i * 5 + 5), "text": f"Line number {i}."}
...
def good_clips() -> List[Dict[str, Any]]:
    return [
        {"rank": 1, "start": "00:30", "end": "01:15", "duration_sec": 45,
         "hook": "The thesis payoff", "why": "high impact", "score": 95},
        {"rank": 2, "start": "01:15", "end": "02:00", "duration_sec": 45,
         "hook": "A twist", "why": "curiosity", "score": 80},
    ]
...
def test_user_prompt_embeds_schema_clamp_and_transcript():
    user_p = build_user_prompt("make shorts", count=4, min_sec=20, max_sec=60,
                               body="[00:00] hello world")
...
def test_rerank_prompt_is_global_and_carries_schema():
    rr = build_rerank_user_prompt("topic", count=5, min_sec=20, max_sec=60,
                                  shortlist_body="[00:30-01:15] score=95")
    low = rr.lower()
...
def test_to_candidates_maps_to_contract_schema_fields(good_clips):
    ...
    # here, by design).
    assert set(c.keys()) == {
        "rank", "start", "end", "durationSec", "hook", "why", "score", "sourceStart",
        "factors", "factorNotes",
    }
    assert set(c["factors"].keys()) == {
        "hookStrength", "emotionalFlow", "perceivedValue", "shareability",
    }
...
def test_select_defaults_count_to_five_when_omitted():
    six = [
        {"start": f"0{i}:00", "end": f"0{i}:40", "score": 90 - i, "hook": "h", "why": "w"}
        for i in range(6)
    ]
