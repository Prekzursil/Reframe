...
class TestEdgeTts:
    ...
    def test_synth_uses_factory_and_ffmpeg_seam(self, tmp_path):
        ...
        engine = et.EdgeTtsEngine(
            communicate_factory=factory, run=fake_run, settings_provider=lambda: SETTINGS
        )
        out = engine.synth(
            CUES, "en-US-AriaNeural", "en-US", str(tmp_path / "o.wav"), rate=1.1
        )
        assert record["args"] == (
...
class TestChatterbox:
    ...
    def test_synth_runs_runner_and_returns_wav(self, tmp_path):
        ...
        engine = cb.ChatterboxEngine(
            env_dir=str(env_dir), python_exe="C:/py/python.exe", run_cmd=fake_run_cmd
        )
    ...
    def test_missing_env_raises_with_asset_hint(self, tmp_path):
        ...
        sample.write_bytes(b"RIFF")
        engine = cb.ChatterboxEngine(
            env_dir=str(tmp_path / "nope"), run_cmd=lambda a, e=None: (0, "")
        )
    ...
    def test_runner_exited_zero_without_wav_raises(self, tmp_path):
        ...
        sample.write_bytes(b"RIFF")
        engine = cb.ChatterboxEngine(
            env_dir=str(env_dir), run_cmd=lambda a, e=None: (0, "silent")
        )
...
class TestChatterboxRunner:
    ...
    def test_parse_job_valid(self):
        job = cbr.parse_job(
            cb.build_job_payload(CUES, "C:/s.wav", "en", "C:/o.wav", 1.0)
        )
