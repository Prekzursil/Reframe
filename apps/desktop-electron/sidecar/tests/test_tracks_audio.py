...
class TestModel:
    ...
    def test_voice_kept_only_when_present(self):
        with_voice = ta.normalize_audio_track(
            {"kind": "dub", "voice": "af_sarah", "path": "x"}
        )
...
class TestArgvBuilders:
    ...
    def test_replace_swaps_exactly_one_stream(self):
        argv = ta.build_replace_argv(
            "in.mkv", "new.m4a", "out.mkv", stream_index=1, lang="en", settings=SETTINGS
        )
        pairs = [(argv[i], argv[i + 1]) for i in range(len(argv) - 1)]
        assert ("-map", "0") in pairs
    ...
    def test_strip_drops_exactly_one_stream_no_second_input(self):
        argv = ta.build_strip_audio_argv(
            "in.mkv", "out.mkv", stream_index=0, settings=SETTINGS
        )
        pairs = [(argv[i], argv[i + 1]) for i in range(len(argv) - 1)]
        assert ("-map", "0") in pairs
...
class DiskStore:
    ...
    def save(self, video_id: str, data: Dict[str, Any]) -> None:
        ...
        self._path(video_id).write_text(
...
class TestService:
    ...
    def test_mux_runs_ffmpeg_and_round_trips_manifest(self, tmp_path):
        ...
        result = service.mux(
            {"videoId": "v1", "path": str(dub), "lang": "de", "name": "German dub",
             "kind": "dub"},
            ctx(),
        )
        ...
        pairs = [(a, b) for a, b in zip(argvs[0], argvs[0][1:])]
    ...
    def test_mux_missing_audio_file_rejected(self, tmp_path):
        ...
        with pytest.raises(RpcError, match="not found"):
            service.mux(
                {"videoId": "v1", "path": str(tmp_path / "ghost.m4a"), "lang": "de",
                 "name": "x", "kind": "dub"},
                ctx(),
            )
    ...
    def test_mux_ffmpeg_failure_surfaces_and_does_not_persist(self, tmp_path):
        ...
        with pytest.raises(RpcError, match="mux failed"):
            service.mux(
                {"videoId": "v1", "path": str(dub), "lang": "de", "name": "x",
                 "kind": "dub"},
                ctx(),
            )
    ...
    def test_replace_updates_path_and_uses_stream_index(self, tmp_path):
        ...
        service, disk, _ = make_service(
            tmp_path, run=lambda argv, **kw: argvs.append(list(argv)) or 0
        )
        dub1 = tmp_path / "a.m4a"
        dub1.write_bytes(b"a")
        service.mux(
            {"videoId": "v1", "path": str(dub1), "lang": "de", "name": "d",
             "kind": "dub"},
            ctx(),
        )
    ...
    def test_replace_unknown_track_rejected(self, tmp_path):
        ...
        with pytest.raises(RpcError, match="no such audio track"):
            service.replace(
                {"videoId": "v1", "audioTrackId": "ghost", "path": str(dub)}, ctx()
            )
    ...
    def test_mux_for_dub_records_voice(self, tmp_path):
        ...
        dub = tmp_path / "dub.m4a"
        dub.write_bytes(b"aac")
        track = service.mux_for_dub(
            "v1", str(dub), lang="de", name="German dub", voice="af_sarah"
        )
