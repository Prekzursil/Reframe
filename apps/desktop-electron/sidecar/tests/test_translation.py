...
class FakeProvider:
    ...
    def __init__(self, *, prefix: str = "XX", fail_all: bool = False, fail_at: Optional[int] = None):
        self.prefix = prefix
...
def test_hosted_factory_returning_none_is_unavailable():
    t = TieredTranslator(
        runner=None, settings={}, hosted_provider_factory=lambda: None
    )
    with pytest.raises(TranslationError) as exc_info:
        t.translate(cues2(), "yo")
...
def test_line_translator_translates_and_binds_tier_once():
    ...
    t = TieredTranslator(
        runner=runner, settings=SETTINGS, local_provider_factory=factory
    )
...
def test_real_runner_model_switch_through_fallback():
    ...
    t = TieredTranslator(
        runner=runner,
        settings=SETTINGS,
        local_provider_factory=make_factory(
            [FakeProvider(fail_all=True), FakeProvider(prefix="T2")]
        ),
    )
    out = t.translate(cues2(), "es")
    assert [c["text"] for c in out] == ["T2:hello there", "T2:good night"]
