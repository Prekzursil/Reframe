...
class RecordingStages:
    ...
    def __init__(
        ...
        calls: List[str],
        *,
        select_return: Optional[List[Dict[str, Any]]] = None,
        snap_impl=None,
        reframe_impl=None,
        filler_impl=None,
    ):
    ...
    def remove_fillers(
        self, in_path, out_path, words, cues, *, lang=None, settings=None
    ):
    ...
    def apply_zoom(
        self, in_path, out_path, cues, *, source_start, duration_sec, settings=None
    ):
    ...
    def brand_overlay(self, in_path, out_path, logo_path, *, settings=None):
        ...
        self.brand_kwargs.append(
            {"in_path": in_path, "out_path": out_path, "logo_path": logo_path}
        )
        return out_path
...
def make_ctx(job_id: str = "t-1") -> JobContext:
    ...
    ctx = JobContext(
        job_id=job_id, _cancel_event=threading.Event(), _emit_progress=emit
    )
...
def loader_for(path: str, transcript: Optional[Dict[str, Any]], **extra):
...
def test_run_export_propagates_sourceStart_to_caption(transcript, tmp_path):
    ...
    rec = RecordingStages([])
    candidate = {
        "rank": 3, "start": 12.5, "end": 50.0, "durationSec": 37.5,
        "hook": "h", "why": "w", "score": 77, "sourceStart": 12.5,
    }
    sm.run_export(
        make_ctx(),
        video_id="v1",
...
def test_run_export_cut_uses_sourceStart_and_end(transcript, tmp_path):
    rec = RecordingStages([])
    candidate = {
        "rank": 1, "start": 99.0, "end": 130.0, "sourceStart": 100.0, "score": 1,
    }
    sm.run_export(
        make_ctx(),
        video_id="v1",
...
class TestCaptionStageRouting:
    def _route(self, monkeypatch, style):
        ...
        out = sm._lazy_caption(
            "clip.mp4", [], "out.mp4",
            source_start=5.0, burn=True, width=1080, height=1920,
...
DUB_TRACK = {
    "id": "aud-dub-1", "lang": "ro", "name": "Romanian dub",
    "kind": "dub", "voice": "kokoro:ro", "path": "/dubs/ro.m4a",
}
ORIGINAL_TRACK = {
    "id": "aud-orig-0", "lang": "en", "name": "Audio 1",
    "kind": "original", "path": "/videos/talk.mp4",
}
...
class TestBuildAudioMuxArgv:
    ...
    def test_maps_clip_video_and_track_audio_window(self):
        argv = sm.build_audio_mux_argv(
            "C:/out/clip 1.mp4", "/dubs/ro.m4a", "C:/out/final 1.mp4",
            start=97.0, end=131.0,
        )
        ...
        assert ["-map", "0:v"] == argv[argv.index("-map"): argv.index("-map") + 2]
        ...
        assert ["-progress", "pipe:1"] == argv[argv.index("-progress"): argv.index("-progress") + 2]
        assert argv[-1] == "C:/out/final 1.mp4"

    def test_stream_index_selects_the_container_audio_lane(self):
        argv = sm.build_audio_mux_argv(
            "clip.mp4", "/videos/talk.mp4", "out.mp4",
            start=10.0, end=40.0, stream_index=2,
        )
    ...
    def test_rejects_inverted_window_and_negative_index(self):
        ...
        with pytest.raises(ValueError):
            sm.build_audio_mux_argv(
                "c.mp4", "a.m4a", "o.mp4", start=0.0, end=10.0, stream_index=-1
            )
...
class TestExportWithAudioTrack:
    def _export(self, tmp_path, transcript, *, audio_track_id, tracks):
        ...
        rec = RecordingStages(calls)
        candidate = {
            "rank": 1, "start": 97.0, "end": 131.0, "durationSec": 34.0,
            "hook": "h", "why": "w", "score": 95, "sourceStart": 97.0,
        }
        out = sm.run_export(
            make_ctx(),
            video_id="v1",
            candidates=[candidate],
            load_context=loader_for(
                str(tmp_path / "talk.mp4"), transcript, audioTracks=tracks
            ),
            out_dir=str(tmp_path / "out"),
            stages=rec.as_stages(),
    ...
    def test_mux_runs_last_and_final_path_is_unchanged(self, transcript, tmp_path):
        out, rec, calls = self._export(
            tmp_path, transcript, audio_track_id="aud-dub-1", tracks=[DUB_TRACK]
        )
    ...
    def test_no_audio_track_id_skips_the_mux_stage(self, transcript, tmp_path):
        _out, rec, calls = self._export(
            tmp_path, transcript, audio_track_id=None, tracks=[DUB_TRACK]
        )
        assert calls == ["cut", "reframe", "caption", "export"]
    ...
    def test_unknown_audio_track_id_fails_before_any_stage(self, transcript, tmp_path):
        ...
        with pytest.raises(ValueError, match="unknown audio track"):
            self._export(
                tmp_path, transcript, audio_track_id="ghost", tracks=[DUB_TRACK]
            )
    ...
    def test_mux_runs_per_clip_for_a_batch(self, transcript, tmp_path):
        ...
        sm.run_export(
            ...
            candidates=cands,
            load_context=loader_for(
                str(tmp_path / "talk.mp4"), transcript, audioTracks=[DUB_TRACK]
            ),
            out_dir=str(tmp_path / "out"),
            stages=rec.as_stages(),
        ...
        assert calls == [
            "cut", "reframe", "caption", "export", "mux_audio",
            "cut", "reframe", "caption", "export", "mux_audio",
        ]
...
class TestExportHandlerAudioTrackParam:
    ...
    def test_export_threads_audioTrackId_to_the_mux_stage(
        self, registry, transcript, tmp_path
    ):
        calls: List[str] = []
        rec = RecordingStages(calls)
    ...
    def test_export_treats_empty_audioTrackId_as_absent(
        self, registry, transcript, tmp_path
    ):
        calls: List[str] = []
        rec = RecordingStages(calls)
...
class TestLazyMuxAudioStage:
    def test_builds_argv_from_the_track_and_runs_it(self, monkeypatch):
        ...
        monkeypatch.setattr(ffmpeg, "ffmpeg_path", lambda settings=None: "/bin/ffmpeg")
        monkeypatch.setattr(
            ffmpeg, "run", lambda argv, **kw: ran.setdefault("argv", argv) and 0 or 0
        )
        track = {**DUB_TRACK, "streamIndex": 0}
        out = sm._lazy_mux_audio(
            "clip.mp4", track, "final.mp4", start=10.0, end=40.0, settings={}
        )
    ...
    def test_track_without_a_path_raises(self):
        with pytest.raises(ValueError, match="no path"):
            sm._lazy_mux_audio(
                "clip.mp4", {"id": "x", "kind": "dub"}, "final.mp4",
                start=0.0, end=10.0,
            )
...
class TestExportRemoveFillers:
    ...
    def _candidate(self):
        return {
            "rank": 1, "start": 20.0, "end": 45.0, "durationSec": 25.0,
            "hook": "the hook", "why": "w", "score": 95, "sourceStart": 20.0,
        }
    ...
    def test_filler_stage_runs_after_cut_when_on(self, transcript, tmp_path):
        ...
        assert calls == [
            "cut", "remove_fillers", "reframe", "caption", "export",
        ]
    ...
    def test_filler_remapped_cues_and_zero_source_start_reach_caption(
        ...
        assert [
            {k: c[k] for k in ("index", "start", "end", "text")}
            for c in cap["cues"]
        ] == sentinel_cues
...
class TestExportHookTitle:
    def _candidate(self, hook="As it turns out, there is a pattern"):
        return {
            "rank": 1, "start": 0.0, "end": 25.0, "durationSec": 25.0,
            "hook": hook, "why": "w", "score": 9, "sourceStart": 0.0,
        }
    ...
    def test_hook_threads_to_caption_by_default(self, transcript, tmp_path):
        ...
        assert rec.caption_kwargs[0]["hook_title"] == (
...
class TestExportTogglesFlowToSettings:
    ...
    def test_removeFillers_true_triggers_the_filler_stage(
        self, registry, transcript, tmp_path
    ):
        calls: List[str] = []
        rec = RecordingStages(calls)
        ...
        candidate = {
            "rank": 1, "start": 0.0, "end": 25.0, "sourceStart": 0.0,
            "hook": "h", "score": 9,
        }
        out = maker.export(
            {
    ...
    def test_emphasis_param_threads_into_settings(
        ...
        candidate = {
            "rank": 1, "start": 0.0, "end": 30.0, "sourceStart": 0.0,
            "hook": "h", "score": 9,
        }
    ...
    def test_autoZoom_param_threads_into_settings(
        self, registry, transcript, tmp_path
    ):
    ...
    def test_autoZoom_default_off_when_absent_from_params(
        self, registry, transcript, tmp_path
    ):
    ...
    def test_toggles_default_when_absent_from_params(
        self, registry, transcript, tmp_path
    ):
        calls: List[str] = []
        rec = RecordingStages(calls)
...
class TestLazyRemoveFillersStage:
    def test_builds_cutlist_runs_argv_and_remaps_cues(self, monkeypatch):
        ...
        out_path, remapped, stats = sm._lazy_remove_fillers(
            "in.mp4", "out.mp4", words, cues, lang="en", settings={}
        )
    ...
    def test_no_keeps_degenerate_returns_input_unchanged(self, monkeypatch):
        ...
        monkeypatch.setattr(
            fillers, "build_cutlist_with_stats", lambda *a, **k: ([], {})
        )
        cues = [{"index": 1, "start": 0.0, "end": 1.0, "text": "x"}]
        out_path, remapped, stats = sm._lazy_remove_fillers(
            "in.mp4", "out.mp4", [], cues, lang="en", settings={}
        )
...
class TestExportWritesShortMetadata:
    ...
    def _candidate(self, **over):
        cand = {
            "rank": 1, "start": 0.0, "end": 25.0, "durationSec": 25.0,
            "hook": "A bold hook", "why": "w", "score": 9, "sourceStart": 0.0,
            "viralityPct": 87,
        }
        cand.update(over)
        return cand
    ...
    def test_metadata_json_written_next_to_mp4(self, transcript, tmp_path):
        ...
        sm.run_export(
            ...
            candidates=[self._candidate()],
            load_context=loader_for(
                str(tmp_path / "src.mp4"), transcript, sourceTitle="My Talk"
            ),
            out_dir=str(tmp_path / "out"),
            stages=rec.as_stages(),
    ...
    def test_metadata_fields_match_section3_schema(self, transcript, tmp_path):
        ...
        meta = json.loads(
            shorts_mod.metadata_path(tmp_path / "out" / "src-1.mp4").read_text("utf-8")
        )
    ...
    def test_metadata_hook_falls_back_to_candidate_when_hooktitle_off(
        self, transcript, tmp_path
    ):
    ...
    def test_listing_reconstructs_short_from_written_metadata(
        self, transcript, tmp_path
    ):
