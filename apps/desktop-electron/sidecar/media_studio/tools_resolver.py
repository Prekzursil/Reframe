...
# chain helpers
# --------------------------------------------------------------------------- #
def _as_executable(candidate: Optional[str], exe_name: str) -> Optional[str]:
...
def _tools_root(root: Optional[str | os.PathLike] = None) -> Path:
...
def resolve_ffmpeg_tool(
    name: str, settings: Optional[Dict[str, Any]] = None
) -> Optional[str]:
...
def require_tool(
    ...
    *,
    ...
    hints = {
        "llama-server": (
            f"set settings.{SETTING_LLAMA_SERVER} or install the "
            f"'{LLAMA_CUDA_ASSET}' / '{LLAMA_CPU_ASSET}' assets"
        ),
        "node-runner": (
            f"set settings.{SETTING_NODE_RUNNER} or launch through the app "
            f"(the supervisor injects {ENV_NODE_RUNNER})"
        ),
...
_LLAMA_BASE_URL = (
    f"https://github.com/ggml-org/llama.cpp/releases/download/{LLAMA_RELEASE_TAG}"
)
...
def _detect_in_tool_dir(sub: str, file_name: str) -> Optional[str]:
    """An extracted file under the CURRENT assets root (env-overridable)."""
    cand = default_config_dir() / sub / file_name
    return str(cand) if cand.is_file() else None


def detect_llama_cuda(settings: Dict[str, Any]) -> Optional[str]:
...
def detect_llama_cpu(settings: Dict[str, Any]) -> Optional[str]:
    """Installed-detection for the CPU build (extracted exe only)."""
    return _detect_in_tool_dir(TOOL_DIR_CPU, LLAMA_EXE)


def detect_llama_cudart(settings: Dict[str, Any]) -> Optional[str]:
