...
class Job:
    ...
    gpu: bool = False
    request: Optional[Dict[str, Any]] = field(default=None, repr=False)
...
class JobRegistry:
    ...
    def create(
        ...
        *,
        feature: str = "",
        label: str = "",
        videoId: Optional[str] = None,  # noqa: N803 - wire-name kwarg per the A2/A3 spec
        gpu: bool = False,
    ) -> Job:
    ...
    def start(
        ...
        *,
        ...
        """
        job = self.create(
            handler, feature=feature, label=label, videoId=videoId, gpu=gpu
        )
        with self._lock:
            job._scheduled = True
    ...
    def cancel(self, job_id: str) -> bool:
        ...
        """
        finish_queued: Optional[Job] = None
        with self._lock:
            job = self._jobs.get(job_id)
            if job is None:
                return False
        ...
        while True:
            with self._lock:
                waiting = [
                    ...
                    if j._scheduled and not j._done_event.is_set()
                ]
            if not waiting:
                break
            rem = remaining()
