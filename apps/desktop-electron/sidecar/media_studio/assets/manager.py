...
RunCmd = Callable[[Sequence[str], Optional[Dict[str, str]]], Tuple[int, str]]
# HF snapshot seam: (repo_id, revision) -> downloaded path (str).
HfFetch = Callable[[str, Optional[str]], str]
...
def parse_total_bytes(
    status_code: int, headers: Mapping[str, str], offset: int
) -> Optional[int]:
...
def hf_cache_dir(env_vars: Optional[Mapping[str, str]] = None) -> Path:
...
def hf_repo_dir(repo_id: str, env_vars: Optional[Mapping[str, str]] = None) -> Path:
...
def _default_run_cmd(
    argv: Sequence[str], extra_env: Optional[Dict[str, str]] = None
) -> Tuple[int, str]:
...
def _default_hf_fetch(repo_id: str, revision: Optional[str]) -> str:
...
class AssetManager:
    ...
    def __init__(
        ...
        *,
        ...
        hf_fetch: Optional[HfFetch] = None,
    ...
    def installed_path(self, entry: AssetEntry) -> Optional[str]:
    ...
    # -- installers -----------------------------------------------------------
    def _install(
        self, entry: AssetEntry, *, on_frac: FracCb, should_cancel: CancelProbe
    ) -> None:
    ...
    def _finalize(
        self, part: Path, dest: Path, sha256: Optional[str], name: str
    ) -> None:
    ...
    def _install_env(
        ...
        steps = build_env_install_argvs(
            self._python_exe, get_pip, env_dir, entry.requirements
        )
        for index, step in enumerate(steps):
            ...
            if returncode != 0:
                ...
                raise AssetError(
                    f"env install failed for {entry.name} "
                    f"(step {index + 1}, exit {returncode}): {tail}"
                )
        sentinel = env_sentinel_path(env_dir)
        sentinel.write_text(
            json.dumps(
