...
class VoiceStore:
    ...
    def __init__(
        self,
        samples_dir: Optional[str | os.PathLike] = None,
        *,
        duration_probe: Optional[DurationProber] = None,
    ) -> None:
        self.samples_dir = (
            Path(samples_dir) if samples_dir is not None else default_voices_dir()
        )
        self.index_path = self.samples_dir / _INDEX_FILENAME
        self._probe = duration_probe or _default_probe

    # -- index I/O -----------------------------------------------------------
    def _load(self) -> List[VoiceSample]:
    ...
    def _save(self, samples: List[VoiceSample]) -> None:
    ...
    def get(self, sample_id: str) -> Optional[VoiceSample]:
        for sample in self._load():
            if sample["id"] == sample_id:
                return sample
        return None

    def add(self, path: str, name: Optional[str] = None) -> VoiceSample:
        ...
        if src.suffix.lower() not in _SAMPLE_SUFFIXES:
            raise TtsError(
                f"unsupported sample format {src.suffix!r} "
                f"(expected one of {', '.join(_SAMPLE_SUFFIXES)})"
            )
        sample_id = _new_id()
        self.samples_dir.mkdir(parents=True, exist_ok=True)
