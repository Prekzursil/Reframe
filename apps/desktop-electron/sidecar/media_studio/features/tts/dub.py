...
class Translator(Protocol):
    ...
    def translate(
        self, texts: Sequence[str], target_lang: str, source_lang: Optional[str] = None
    ) -> List[str]:
...
def build_encode_aac_argv(
    in_wav: str, out_m4a: str, settings: Optional[Dict[str, Any]] = None
) -> List[str]:
...
class DubService:
    ...
    def __init__(
        ...
        *,
        ...
        run: RunFn = ffmpeg.run,
        duration: DurationFn = wav_duration_sec,
        media_duration: Optional[Callable[[str], float]] = None,
    ...
    def _resolve_voice(
        self, engine_id: str, voice: Optional[str], sample_id: Optional[str]
    ) -> str:
        """Map the wire's voice?/sampleId? onto the engine's ``voice`` arg."""
        if engine_id == "chatterbox":
            if not sample_id:
                raise RpcError(
                    "sampleId is required for the chatterbox engine "
                    "(add one via tts.sample.add)",
                    ErrorCode.INVALID_PARAMS,
                )
            sample = self._voice_store.get(sample_id)
            if sample is None:
                raise RpcError(
                    f"unknown sampleId: {sample_id}", ErrorCode.INVALID_PARAMS
                )
    ...
    def dub_start(self, params: Dict[str, Any], ctx: RpcContext) -> Dict[str, Any]:
        ...
        if factory is None:
            raise RpcError(
                ...
                f"(expected one of {', '.join(sorted(self._engines))})",
                ErrorCode.INVALID_PARAMS,
            )
        ...
        voice = self._resolve_voice(
            engine_id, params.get("voice"), params.get("sampleId")
        )
        ...
        def job_body(job_ctx: JobContext) -> Dict[str, Any]:
            ...
            translator = (
                self._translator_factory() if self._translator_factory else None
            )
            total = None
