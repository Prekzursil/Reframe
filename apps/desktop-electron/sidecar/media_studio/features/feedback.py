...
def _factor_average(candidate: Dict[str, Any]) -> Optional[float]:
...
class FeedbackStore:
    ...
    # ------------------------------------------------------------------ I/O
    def record(
        self, video_id: str, candidate: Dict[str, Any], action: str
    ) -> Dict[str, Any]:
        ...
        if action not in ACTIONS:
            raise ValueError(
                f"action must be one of {list(ACTIONS)}, got {action!r}"
            )
    ...
    def exemplar_block(self, language: Optional[str] = None) -> Optional[str]:
        ...
        if language:
            ...
            matched = [
                e for e in entries
                if str(e.get("candidate", {}).get("language", "")).strip().lower()
                == lang
            ]
        ...
        lines: List[str] = [
            "TASTE CALIBRATION — this user's past labels on clips like these:"
        ]
        if approved:
        ...
        lines.append(
            "Prefer clips in the spirit of the approved hooks; avoid the "
            "discarded kind."
        )
        block = "\n".join(lines)
        if len(block) > EXEMPLAR_MAX_CHARS:  # hard token budget (~400 tokens)
            block = block[: EXEMPLAR_MAX_CHARS - 1].rstrip() + "…"
        return block

    # ------------------------------------------------------------ calibration
    def calibration_table(self) -> Optional[List[Optional[float]]]:
