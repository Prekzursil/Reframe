...
class FeedbackSeam(Protocol):
    ...
    def exemplar_block(self, language: Optional[str] = None) -> Optional[str]:
        ...

    def calibrated_pct(self, raw: float) -> Optional[int]:
        ...
...
def build_user_prompt(
    user_prompt: str, count: int, min_sec: float, max_sec: float, body: str
) -> str:
    ...
    return (
        f"{user_prompt}\n\n"
        f"Select the {n} best clips.\n{schema}\n\n"
        f"Transcript:\n{body}"
    )


def build_rerank_user_prompt(
    user_prompt: str, count: int, min_sec: float, max_sec: float, shortlist_body: str
) -> str:
...
def parse_factors(clip: Dict[str, Any], score: int) -> Dict[str, int]:
    ...
    return {
        name: _clamp_0_100(raw.get(name, fallback), fallback)
...
def parse_factor_notes(clip: Dict[str, Any]) -> Dict[str, str]:
    ...
    raw = raw if isinstance(raw, dict) else {}
    return {
        name: str(raw.get(name, "") or "")[:_FACTOR_NOTE_MAX_CHARS]
        for name in FACTOR_NAMES
    }
...
def to_candidates(
    ...
    ordered = sorted(
        enumerate(parsed), key=lambda iv: (-iv[1]["score"], iv[0])
    )
...
def _resolve_controls(controls: Optional[Controls]) -> Dict[str, Any]:
...
def select(
    ...
    user_prompt = (prompt or "").strip() or (
        "Find the most share-worthy clips for vertical short-form."
    )
    ...
    if len(lines) <= CHUNK_LINE_THRESHOLD:
        body = "\n".join(lines)
        clips = _ask(provider, system, build_user_prompt(
            user_prompt, count, min_sec, max_sec, body))
    ...
    for chunk in _chunk_lines(lines, CHUNK_LINE_SIZE):
        body = "\n".join(chunk)
        clips = _ask(provider, chunk_system, build_user_prompt(
            user_prompt, CHUNK_SHORTLIST_COUNT, min_sec, max_sec, body))
        shortlist.extend(
            to_candidates(clips, min_sec, max_sec, duration_total)
        )
    ...
    rerank_clips = _ask(provider, system, build_rerank_user_prompt(
        user_prompt, count, min_sec, max_sec, shortlist_body))
