...
def normalize_audio_track(track: Dict[str, Any]) -> AudioTrack:
    ...
    if kind not in _KINDS:
        raise AudioTrackError(
            f"audioTrack.kind must be 'original' or 'dub', got {kind!r}"
        )
...
def remove_audio_track(project: Dict[str, Any], track_id: str) -> AudioTrack:
    ...
    project["audioTracks"] = [
        ...
        if not (isinstance(t, dict) and t.get("id") == track_id)
    ]
    return track
...
class AudioTracksService:
    ...
    def __init__(
        ...
        *,
        ...
        duration: Optional[DurationFn] = None,
    ...
    def _seed_originals(
        self, project: Dict[str, Any], video_id: str, video_path: str
    ) -> bool:
        """Seed 'original' rows from ffprobe on first contact. Returns changed?"""
        tracks = audio_tracks_of(project)
        if any(
            isinstance(t, dict) and t.get("kind") == KIND_ORIGINAL for t in tracks
        ):
            return False
        try:
    ...
    def _run_or_raise(self, argv: List[str], in_path: str, what: str) -> None:
        total = 0.0
        try:
        ...
        if code != 0:
            raise RpcError(
                f"{what} failed (ffmpeg exit {code})", ErrorCode.INTERNAL_ERROR
            )
    ...
    def strip(self, params: Dict[str, Any], ctx: RpcContext) -> Dict[str, Any]:
        ...
        argv = build_strip_audio_argv(
            video_path, out_path, stream_index=index, settings=self._settings()
        )
