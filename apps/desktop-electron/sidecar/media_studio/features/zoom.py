...
def sentence_start_beats(
    cues: Sequence[CueLike], *, source_start: float = 0.0
) -> List[float]:
...
def _punch_term(beat: float, fps: int) -> str:
    ...
    return (
        f"{PUNCH_ZOOM:.6f}*gte(on/{fps},{beat:.3f})"
        f"*min((on/{fps}-{beat:.3f})/{ramp:.3f}\\,1)"
    )
...
def build_zoom_filter(
    *,
    ...
    cues: Optional[Sequence[CueLike]] = None,
    ...
    if beats is not None:
        ...
        resolved_beats = []
    z_expr = build_zoom_expr(
        duration_sec=duration_sec, beats=resolved_beats, fps=fps
    )
