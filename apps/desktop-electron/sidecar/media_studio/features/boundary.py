...
def build_boundary_set(
    words: Sequence[Word],
    *,
    silences: Optional[Sequence[float]] = None,
    scene_cuts: Optional[Sequence[float]] = None,
    silence_provider: Optional[SilenceProvider] = None,
    ...
    def _clean(values: Optional[Sequence[float]]) -> Tuple[float, ...]:
        if not values:
            return ()
...
def snap_candidate(
    ...
    *,
    ...
    snapped = _snap_pair(
        orig_start, orig_end, targets, spans, min_sec=min_sec, max_sec=max_sec
    )
    if snapped is None:
        return SnapResult(
            candidate=None,
            dropped=True,
            reason=(
                "no valid boundary keeps the clip within "
                f"{min_sec:g}-{max_sec:g}s without cutting mid-word"
            ),
        )
...
def _snap_pair(
    ...
    *,
    ...
    start_a = _nearest_valid(
        orig_start, targets, lower=0.0, upper=orig_end, spans=spans
    )
    ...
    end_b = _nearest_valid(
        orig_end, targets, lower=orig_start, upper=_upper_bound(targets, orig_end), spans=spans
    )
...
def snap_candidates(
    ...
    *,
    ...
    for cand in candidates:
        result = snap_candidate(
            cand, words, boundaries, min_sec=min_sec, max_sec=max_sec
        )
...
def snap_from_lists(
    ...
    *,
    ...
    return snap_candidates(
        candidates, words, boundaries, min_sec=min_sec, max_sec=max_sec
    )
...
def detect_silences(
    ...
    *,
    ...
    noise_db: float = DEFAULT_SILENCE_NOISE_DB,
    min_silence_sec: float = DEFAULT_SILENCE_MIN_SEC,
    run: Optional[SilenceRunner] = None,
) -> Tuple[float, ...]:
...
def _default_scene_detect(video_path: str, threshold: float = 27.0) -> Sequence[float]:
    ...
    """
    from scenedetect import detect, ContentDetector  # type: ignore  # pragma: no cover - prod seam
...
def detect_scene_cuts(
    ...
    *,
    ...
    """
    detect_fn = detector if detector is not None else (
        lambda p: _default_scene_detect(p, threshold)
    )
    try:
