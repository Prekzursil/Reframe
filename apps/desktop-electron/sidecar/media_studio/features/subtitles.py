...
def translate(
    ...
    *,
    ...
    translator: Optional[LineTranslator] = None,
    ...
    for i, cue in enumerate(cues):
        ...
        out_cues.append(make_cue(int(cue.get("index", i + 1)), float(cue.get("start", 0.0)), float(cue.get("end", 0.0)), new_text))
...
def _parse_dialogue(line: str, fmt_cols: List[str]) -> Optional[Cue]:
...
def load(in_path: str | Path, fmt: Optional[str] = None) -> List[Cue]:
