...
def build_keyframes(timestamps: Sequence[float], xs: Sequence[int]) -> List[Dict[str, float]]:
    ...
    return [{"t": float(t), "x": int(x)} for t, x in zip(timestamps, xs)]


def dedupe_keyframes(
    keyframes: List[Dict[str, float]], min_delta: float
) -> List[Dict[str, float]]:
...
def build_crop_x_expr(
    static_x: int, keyframes: Optional[Sequence[Dict[str, float]]] = None
) -> str:
    ...
    for prev, nxt in zip(reversed(kfs[:-1]), reversed(kfs[1:])):
...
def build_reframe_argv(
    ...
    out_path: str,
    crop: Dict[str, int],
    keyframes: Optional[Sequence[Dict[str, float]]] = None,
...
def probe_video(
    ...
    try:
        ...
        raise ClaudeShortsReframeError(
            f"could not probe video geometry for {in_path}"
        ) from exc
...
def _make_face_finder(backend: str) -> Tuple[Optional[Callable[[Any], Optional[float]]], Callable[[], None]]:
    ...
    if backend == "mediapipe":
        ...
        def find_mp(img: Any) -> Optional[float]:
    ...
    if backend == "haar":
        ...
        def find_haar(img: Any) -> Optional[float]:
...
class ClaudeShortsReframeEngine:
    ...
    def __init__(
        ...
        self._prober: Prober = prober or (
            lambda path: probe_video(path, self._settings)
        )
    ...
    def reframe(
        ...
        argv = build_reframe_argv(
            in_path, out_path, crop, keyframes, aspect, self._settings
        )
        ...
        if code != 0:
            raise ClaudeShortsReframeError(
                f"claudeshorts reframe failed (exit {code}) for {out_path}"
            )
        return out_path
