...
def _words_of(transcript: Optional[Transcript]) -> List[Dict[str, Any]]:
...
def _lazy_snap(
    candidates, transcript, *, settings=None
...
def _lazy_zoom(
    in_path, out_path, cues, *, source_start, duration_sec, settings=None
) -> str:
...
def _lazy_brand_overlay(in_path, out_path, logo_path, *, settings=None) -> str:
    ...
    argv = _brandkit.build_logo_overlay_argv(
        in_path, logo_path, out_path, settings=settings
    )
    code = _ffmpeg.run(argv)
    if code != 0:  # pragma: no cover - prod seam
...
def _lazy_export(in_path, out_path, *, settings=None) -> str:
    ...
    argv = _ffmpeg.build_convert_argv(
        in_path, out_path, {"vcodec": "libx264", "acodec": "aac"}, settings
    )
    code = _ffmpeg.run(argv)
    if code != 0:  # pragma: no cover - prod seam
...
def _lazy_mux_audio(
    clip_path, audio_track, out_path, *, start, end, settings=None
) -> str:
...
# small helpers (pure)
# ---------------------------------------------------------------------------
def _is_empty_transcript(transcript: Optional[Transcript]) -> bool:
...
def _cues_for_clip(transcript: Optional[Transcript], candidate: Candidate) -> List[Cue]:
...
def _clip_local_words(
    transcript: Optional[Transcript], source_start: float, end: float
) -> List[Dict[str, Any]]:
...
def _rebase_cues(cues: List[Cue], source_start: float) -> List[Cue]:
    ...
    for cue in cues or []:
        ...
        if end <= start:
            continue
        out.append(
            {"index": len(out) + 1, "start": start, "end": end,
...
def _resolve_audio_track(
    context: Dict[str, Any], audio_track_id: str
...
def run_select(
    ...
    *,
    ...
    transcript: Optional[Transcript] = context.get("transcript")
...
# ---------------------------------------------------------------------------
def _candidate_virality(candidate: Candidate) -> Optional[int]:
...
def _export_one(
    ...
    *,
    ...
    filler_stats: Optional[Dict[str, Any]] = None
    ...
    caption_cues = _emphasis.annotate(
        caption_cues, enable=_emphasis.resolve_emphasis(settings)
    )
