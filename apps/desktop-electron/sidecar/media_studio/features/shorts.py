...
def read_metadata(clip_path: str | os.PathLike) -> Optional[Dict[str, Any]]:
...
class Shorts:
    ...
    def __init__(
        ...
        *,
        ...
        out_dir_for: Optional[Callable[[str], str]] = None,
        settings_provider: Optional[Callable[[], Dict[str, Any]]] = None,
        probe: Optional[ProbeFn] = None,
        run: Optional[RunFn] = None,
    ) -> None:
        self._exports_dir = Path(exports_dir)
        self._out_dir_for = out_dir_for or (
            lambda vid: str(self._exports_dir / f"shorts-{vid}")
        )
    ...
    def _guard_in_root(self, path: str) -> Path:
        ...
        try:
            ...
            raise _invalid(f"path is outside the exports root: {path}")
        return resolved

    def _scan_dir(self, directory: Path) -> List[Dict[str, Any]]:
