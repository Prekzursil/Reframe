...
CHROME_HEADLESS_SHELL_EXE_REL = (
    f"tools/chrome-headless-shell-win64/chrome-headless-shell{_EXE}"
)


def detect_chrome_headless_shell(settings: Dict[str, Any]) -> Optional[str]:
...
def ensure_chrome_extracted(zip_path: Path, extract_root: Path) -> Optional[Path]:
    ...
    """
    exe = extract_root / Path(CHROME_HEADLESS_SHELL_EXE_REL).relative_to(
        CHROME_HEADLESS_SHELL_EXTRACT_DIR
    )
    if exe.is_file():
        return exe
            ...
            for member in zf.namelist():
                ...
                if not str(target).startswith(str(root)):
                    raise RemotionCaptionError(
                        f"unsafe zip member path in {zip_path.name}: {member!r}"
                    )
            zf.extractall(root)
...
def build_job(
    ...
    *,
    ...
    if style not in STYLES:
        raise RemotionCaptionError(
            f"unknown caption style {style!r} (expected one of {STYLES})"
        )
...
def build_spawn_env(base_env: Optional[Mapping[str, str]] = None) -> Dict[str, str]:
...
def parse_render_ok(line: str) -> Optional[str]:
...
def parse_render_progress(line: str) -> Optional[float]:
...
def run_render(
    ...
    *,
    ...
    should_cancel: Optional[CancelProbe] = None,
    ...
    * ``should_cancel`` is polled per stdout line; cancellation terminates the
    ...
    if stderr is not None and hasattr(stderr, "__iter__"):
        ...
        drain_thread = threading.Thread(
            target=_drain, daemon=True, name="remotion-stderr"
        )
        drain_thread.start()

    ok_path: Optional[str] = None
...
class RemotionCaptionEngine:
    ...
    def resolve_runtime(self) -> Tuple[str, str, str, Optional[str]]:
        """(node_exe, render_js, bundle_dir, chromium_or_None) for this engine."""
        node_exe = resolve_node_exe(
            self._settings, env=self._env, dev_root=self._dev_root
        )
        render_js = resolve_render_js(
            self._settings, env=self._env, dev_root=self._dev_root
        )
        ...
        chromium = resolve_chromium(
            self._settings, env=self._env, assets_root=self._assets_root
        )
    ...
    def render(
        ...
        if not burn:
            raise RemotionCaptionError(
                "the remotion CaptionEngine only burns captions (use the "
                "libass engine for soft-mux)"
            )
        ...
        fd, job_path = tempfile.mkstemp(
            suffix=".json", prefix="media_studio_remotion_job_"
        )
        try:
            ...
            for attempt in range(1, MAX_SUBPROCESS_ATTEMPTS + 1):
                ...
                if (
                    attempt < MAX_SUBPROCESS_ATTEMPTS
                    and is_transient_compositor_failure(stderr_tail)
                ):
                    log.warning(
