...
class Library:
    ...
    def __init__(self, index_path: str | os.PathLike, probe_duration: Optional[DurationProber] = None):
        self.index_path = Path(index_path)
        self._probe = probe_duration or _default_probe

    # ---- index I/O ---------------------------------------------------------
    def _load(self) -> List[Video]:
    ...
    def _save(self, videos: List[Video]) -> None:
    ...
    def add(self, path: str, title: Optional[str] = None) -> Video:
    ...
    def set_has_transcript(self, video_id: str, value: bool = True) -> Optional[Video]:
        """Mark a video's ``hasTranscript`` flag and persist; returns the Video."""
        videos = self._load()
        result: Optional[Video] = None
        for v in videos:
            if v["id"] == video_id:
...
class Project:
    ...
    def __init__(self, data: Project, manifest_path: Optional[str | os.PathLike] = None):
        self.data = data
        self.manifest_path = Path(manifest_path) if manifest_path else None

    # ---- construction ------------------------------------------------------
    @classmethod
    def new(cls, video: Video, settings: Optional[Dict[str, Any]] = None) -> "Project":
    ...
    # ---- persistence -------------------------------------------------------
    def save(self, manifest_path: Optional[str | os.PathLike] = None) -> Path:
