...
def resolve_binary(name: str, settings: Optional[Dict[str, Any]] = None) -> str:
    ...
    if setting_val:
        ...
        if sp.is_file():
            ...
            if sp.stem.lower() == name.lower() or (
                name == "ffmpeg" and "ffmpegPath" in settings
            ):
                return str(sp)
...
def ffmpeg_path(settings: Optional[Dict[str, Any]] = None) -> str:
    """Absolute path to the ffmpeg binary."""
    return resolve_binary("ffmpeg", settings)


def ffprobe_path(settings: Optional[Dict[str, Any]] = None) -> str:
...
# ---------------------------------------------------------------------------
def build_probe_argv(in_path: str, settings: Optional[Dict[str, Any]] = None) -> List[str]:
...
def _out_time_to_seconds(value: str) -> Optional[float]:
...
def _pct_from_progress(key: str, value: str, total_sec: float) -> Optional[float]:
