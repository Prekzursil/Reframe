...
class RpcServer:
    ...
    def __init__(
        ...
        self.jobs = JobRegistry(
            emit_progress=self._emit_progress, emit_done=self._emit_done
        )
    ...
    def handle_line(self, line: str) -> None:
        ...
        try:
            ...
            self._write_obj(
                make_error(None, RpcError(f"parse error: {exc}", ErrorCode.PARSE_ERROR))
            )
            return
        ...
        try:
            ...
            if not req.is_notification:
                wrapped = RpcError(
                    f"internal error in {req.method}: {exc}", ErrorCode.INTERNAL_ERROR
                )
...
def build_server(
    instream: Optional[TextIO] = None, outstream: Optional[TextIO] = None
) -> RpcServer:
