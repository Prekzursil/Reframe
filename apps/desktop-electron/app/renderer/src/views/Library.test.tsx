...
describe('Library', () => {
  it('lists videos from library.list', async () => {
    rpcMock.mockResolvedValueOnce({ videos: [makeVideo(), makeVideo({ id: 'v2', title: 'Second' })] });
    await renderLibrary();
  ...
  it('multi-adds videos picked via the native dialog (window.api.openVideos)', async () => {
    ...
    rpcMock
      .mockResolvedValueOnce({ video: makeVideo({ id: 'a', title: 'Clip A', path: '/clips/a.mp4' }) })
      .mockResolvedValueOnce({ video: makeVideo({ id: 'b', title: 'Clip B', path: '/clips/b.mp4' }) });
  ...
  it('adds dropped files via drag-drop using the pathForFile bridge (Electron >=32)', async () => {
    ...
    rpcMock
      .mockResolvedValueOnce({ video: makeVideo({ id: 'd1', title: 'Drop One', path: '/dropped/a.mp4' }) })
      .mockResolvedValueOnce({ video: makeVideo({ id: 'd2', title: 'Drop Two', path: '/dropped/b.mp4' }) });
  ...
  it('surfaces a per-file typed error toast but continues the batch (bad-file path)', async () => {
    ...
    rpcMock
      .mockRejectedValueOnce(new Error('not a video file'))
      .mockResolvedValueOnce({ video: makeVideo({ id: 'g', title: 'Good', path: '/clips/good.mp4' }) });
