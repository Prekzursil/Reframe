...
describe('Shorts view', () => {
  it('loads shorts.list on mount and renders a card per clip', async () => {
    listMock.mockResolvedValue({
      shorts: [makeShort({ id: 's1', sourceTitle: 'Talk A' }), makeShort({ id: 's2', sourceTitle: 'Talk B' })],
    });
  ...
  it('sort toggle reorders the cards by virality without a refetch (P4 §7)', async () => {
    ...
    const viralityBtn = [
      ...container.querySelectorAll('[aria-label="Sort shorts"] button'),
  ...
  it('Delete confirms, calls shorts.delete with the path, then reloads', async () => {
    listMock
      .mockResolvedValueOnce({ shorts: [makeShort({ id: 's1', path: '/exports/shorts-v1/a.mp4' })] })
  ...
  it('Re-export calls shorts.reexport then the onReexport callback with the hint', async () => {
    listMock.mockResolvedValue({ shorts: [makeShort({ path: '/exports/shorts-v1/a.mp4', videoId: 'v9' })] });
    const hint = {
      videoId: 'v9',
  ...
  it('serves an existing thumbnailPath over mstream without calling shorts.thumbnail', async () => {
    listMock.mockResolvedValue({
      shorts: [makeShort({ path: '/exports/shorts-v1/a.mp4', thumbnailPath: '/exports/shorts-v1/a.thumb.jpg' })],
    });
