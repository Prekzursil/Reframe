...
export function ShortMaker({
  ...
  useEffect(() => {
    ...
    let alive = true;
    Promise.resolve(
      resolvedApi.rpc<{ labels?: unknown; calibrated?: unknown }>('feedback.stats'),
    )
      .then((res) => {
  ...
  const setControl = useCallback(<K extends keyof ShortMakerControls>(
    key: K,
    value: ShortMakerControls[K],
  ) => {
    setControls((prev) => {
  ...
  const runBatch = useCallback(async () => {
    ...
    try {
      ...
      const clips = await resolveJobResult(
        resolvedApi, expRes, extractClips, activeJobRef, EXPORT_JOB_TIMEOUT_MS,
      );
      setExportedClips(clips ?? []);
  ...
  return (
    <section className="shortmaker" aria-label="Short maker">
      ...
      {phase === 'reviewing' && items.length === 0 && !busy && (
        <p className="sm-empty">No candidates were proposed. Adjust the prompt or controls and retry.</p>
      )}
      ...
      {items.length > 0 && (
        <div className="sm-export">
          ...
          <button type="button" onClick={() => void runExport()} disabled={busy || approved.length === 0}>
            Export approved
          </button>
        </div>
      )}
