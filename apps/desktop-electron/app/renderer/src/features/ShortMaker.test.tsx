...
describe('sanitizeControls', () => {
  ...
  it('rejects non-boolean toggle values back to the defaults', () => {
    ...
    expect(
      sanitizeControls({ removeFillers: 'true' as unknown as boolean }).removeFillers,
    ).toBe(false);
  });
  ...
  it('keeps explicit emphasis choices and normalizes case', () => {
    ...
    expect(
      sanitizeControls({ emphasis: 'ON' as unknown as 'on' }).emphasis,
    ).toBe('on');
  });
...
describe('PLATFORM_PRESETS / applyPreset (P4 §8c)', () => {
  ...
  it('applies aspect/count + the §5-clamped maxSec, keeping the other controls', () => {
    const base = sanitizeControls({ captionStyle: 'hormozi', reframeEngine: 'verthor', minSec: 25 });
...
describe('buildExportParams (P4 §8c / §2 export contract)', () => {
  it('carries candidateIds + inline candidates + the controls', () => {
    ...
    const ctrl = sanitizeControls({ captionStyle: 'bold', reframeEngine: 'verthor', hookTitle: false });
  ...
  it('always sends autoZoom (P4 §8b) and reflects an explicit ON', () => {
    ...
    expect(
      buildExportParams('v1', top, sanitizeControls({ autoZoom: true }), '').autoZoom,
    ).toBe(true);
  });
...
describe('readBrandSettings / brandSettingsPatch (P4 §8d)', () => {
  ...
  it('drops an unknown caption template id (never shows a stale default)', () => {
    ...
    expect(readBrandSettings({ brandCaptionTemplate: 'libass' }).brandCaptionTemplate).toBe('libass');
  });
...
describe('<ShortMaker /> component', () => {
  ...
  it('nudge updates the displayed boundaries, reset restores them (non-destructive)', async () => {
    const api = makeApi({
      rpc: vi.fn().mockResolvedValue({ candidates: [cand({ start: 100, end: 140, durationSec: 40 })] }),
    });
    render(<ShortMaker videoId="v1" api={api} />);
    const form = container.querySelector('form')!;
    await act(async () => {
  ...
  ): Api['rpc'] & ReturnType<typeof vi.fn> {
  ...
  it('records one exported feedback action per exported candidate', async () => {
    ...
    expect(
      exported.map((c) => (c[1] as { candidate: Candidate }).candidate.rank).sort(),
    ).toEqual([1, 2]);
  });
  ...
  it('shows per-clip filler-removal stats on the exported list when present', async () => {
    ...
    expect(lis[0].querySelector('.sm-fillers')?.textContent).toContain(
      'removed 4 fillers (2.5s)',
    );
  ...
  it('overlays the live caption (word run + hook title) inside .sm-phone over the Player', async () => {
    const rpc = rpcFake({
      ...
      'shortmaker.select': {
        candidates: [cand({ rank: 1, start: 100, end: 130, durationSec: 30, sourceStart: 100, hook: 'Big idea' })],
      },
      'captions.cues': {
        cues: [
    ...
    render(<ShortMaker videoId="v1" api={makeApi({ rpc })} initialControls={{ captionStyle: 'bold' }} />);
    await submitForm();
  ...
  it('updates the overlay live when the caption-style select changes (none -> rendered)', async () => {
    ...
    render(<ShortMaker videoId="v1" api={makeApi({ rpc })} initialControls={{ captionStyle: 'none' }} />);
    await submitForm();
  ...
  it('remounts the preview Player once the proxy makes the source playable (job.done re-poll)', async () => {
    ...
    const rpc = vi.fn(async (method: string) => {
      ...
      if (method === 'shortmaker.select') {
        return { candidates: [cand({ rank: 1, start: 100, end: 130, durationSec: 30, sourceStart: 100 })] };
      }
  ...
  it('picks a logo via the pickLogoFile bridge and persists it', async () => {
    ...
    const setCall = rpc.mock.calls.find(
      (c) => c[0] === 'settings.set' && (c[1] as { brandLogoPath?: string }).brandLogoPath === '/picked/logo.png',
    );
    expect(setCall).toBeTruthy();
  ...
  it('surfaces an error when the logo picker bridge is absent', async () => {
    ...
    await flush();
    expect(container.querySelector('[role="alert"]')?.textContent).toContain('Logo picker is unavailable');
  });
});
