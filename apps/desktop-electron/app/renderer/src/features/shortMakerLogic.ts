...
export function sanitizeControls(raw: Partial<ShortMakerControls>): ShortMakerControls {
  ...
  const maxSec0 = clamp(Math.round(raw.maxSec ?? DEFAULT_CONTROLS.maxSec), MIN_CLIP_SEC, MAX_CLIP_SEC);
  let maxSec = maxSec0;
  if (minSec > maxSec) {
  ...
  const hookTitle =
    typeof raw.hookTitle === 'boolean' ? raw.hookTitle : DEFAULT_CONTROLS.hookTitle;
  const removeFillers =
  ...
  const autoZoom =
    typeof raw.autoZoom === 'boolean' ? raw.autoZoom : DEFAULT_CONTROLS.autoZoom;
  return {
    count,
    minSec,
    maxSec,
