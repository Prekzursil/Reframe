...
export function Dub({ videoId, api }: DubProps): React.ReactElement {
  ...
  const startDub = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setJobId(id);
      const done = id
        ? await waitForJobDone<unknown>(bridge, id, (r) => r ?? null)
        : null;
  ...
  return (
    <section className="feature-panel dub-panel" aria-label="Dub">
      ...
      <p className="dub-intro">
        Synthesize a dubbed audio track from a subtitle track — locally
        (Kokoro), hosted (Edge TTS, ONLINE) or cloning a voice sample
        (Chatterbox).
      </p>
      ...
      <ul className="audio-track-list">
        {audioTracks.map((t) => (
          <li key={t.id} className="audio-track-row" data-audio-track={t.id}>
            ...
            <span className={`audio-track-kind audio-track-kind--${t.kind}`}>
              {t.kind}
            </span>
      ...
      {audioTracks.length === 0 && (
        <p className="audio-track-empty">No audio tracks yet.</p>
      )}
    </section>
  );
}
