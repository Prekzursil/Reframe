...
export const client = {
  ...
  subtitles: {
    ...
    translate: (trackId: string, targetLang: string): Promise<JobHandle & { track?: SubtitleTrack }> =>
  ...
  media: {
    playable: (videoId: string): Promise<MediaPlayableResult> =>
      rpc('media.playable', { videoId }),
  ...
  tts: {
    ...
    sampleAdd: (path: string): Promise<{ sample: VoiceSample }> =>
