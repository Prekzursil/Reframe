...
import {
  exportShortsTimelineCsv,
  exportShortsTimelineEdl,
  type ShortsClip,
} from "./shorts/timeline";
...
function CaptionsForm({
  ...
  const [diarizationBackend, setDiarizationBackend] = useState<"noop" | "speechbrain" | "pyannote">(
    "noop",
  );
    ...
    <form className="form-grid" onSubmit={submit}>
      ...
      <details className="field full">
        ...
        <div className="form-grid" style={{ marginTop: 12 }}>
          ...
          <label
            className="field"
            title="Model name used by the selected backend. Ignored for 'noop'."
          >
            <span>Model</span>
          ...
          <label
            className="field"
            title="Subtitle segmentation profile tuned for readability or high-impact edits."
          >
          ...
          >
            ...
            <select
              ...
              onChange={(e) => setDiarizationBackend(e.target.value as typeof diarizationBackend)}
            >
          ...
          <div className="field full">
            ...
            {speakerLabelsEnabled && diarizationBackend === "pyannote" && (
              <p className="muted">
                Speaker labels via pyannote require a worker build that includes diarization deps
                (pyannote + torch) and a Hugging Face token (`HF_TOKEN`). Offline mode will skip
                diarization. If the worker can’t diarize, it will fall back without failing the job.
              </p>
            )}
            {speakerLabelsEnabled && diarizationBackend === "speechbrain" && (
              <p className="muted">
                Speaker labels via SpeechBrain are token-free, but still require heavy deps
                (speechbrain + torch + torchaudio) and may download models. Offline mode will skip
                diarization. If the worker can’t diarize, it will fall back without failing the job.
              </p>
            )}
          </div>
...
function UploadPanel({
  ...
  return (
    ...
    >
      ...
      <p className="muted">
        Drop a file here or click to select. Uploads to the backend and returns an asset id.
      </p>
...
function AudioUploadPanel({
  ...
  return (
    ...
    >
      ...
      <p className="muted">
        Drop a file or click to select. Uploads to the backend and returns an asset id.
      </p>
...
function SubtitleEditorCard({
  ...
  return (
    <div className="form-grid">
      <p className="muted">
        Edit subtitles inline and optionally shift all timings, then re-upload as a new subtitle
        asset.
      </p>
      <label className="field">
        <span>Subtitle asset ID</span>
        <div className="actions-row">
          <Input
            ...
            placeholder="Paste an asset id or upload first"
          />
          <Button
            type="button"
            variant="secondary"
            onClick={() => void loadFromAssetId()}
      ...
      <label className="field">
        ...
        <div className="actions-row">
          <Button
            type="button"
            variant={editorMode === "raw" ? "primary" : "secondary"}
        ...
        {editorMode === "cues" && (
          <p className="muted">
            Cue table mode rewrites the subtitle file; use raw mode for advanced VTT styling.
          </p>
        )}
      </label>
      <label className="field">
      ...
      {editorMode === "raw" ? (
        <label className="field full">
          <span>
            Subtitle contents{" "}
            {fmt ? (
              <span className="muted">({fmt.toUpperCase()})</span>
            ) : (
              <span className="muted">(unknown format)</span>
            )}
          </span>
          <TextArea
            rows={14}
        ...
        <div className="field full">
          ...
          <div className="cue-table">
            {cues.length === 0 && (
              <p className="muted">No cues parsed. Switch to raw mode to edit text directly.</p>
            )}
            {cues.map((cue, idx) => (
          ...
          {savedOpenUrl && (
            <div className="actions-row">
              ...
              <Button
                type="button"
                variant="ghost"
                onClick={() => navigator.clipboard.writeText(saved.id).catch(() => {})}
              >
                Copy ID
              </Button>
            </div>
...
function SubtitleToolsForm({
  onCreated,
  projectId,
}: {
  onCreated: (job: Job, bilingual: boolean) => void;
  projectId?: string;
}) {
  const [subtitleId, setSubtitleId] = useState("");
  const [targetLang, setTargetLang] = useState("es");
  ...
  return (
    <form className="form-grid" onSubmit={handleSubmit}>
      ...
      {safeUploadPreview && (
        <TextPreview
          url={safeUploadPreview}
          title={`Uploaded subtitle preview ${uploadName ? `(${uploadName})` : ""}`}
        />
      )}
      {error && <div className="error-inline">{error}</div>}
      <div className="actions-row">
  ...
  const curlCommand = useMemo(() => {
    const payload = {
      ...
      options: {
        ...
        ...(trimSilence
          ? { silence_noise_db: silenceNoiseDb, silence_min_duration: silenceMinDuration }
          : {}),
    ...
    <form className="form-grid" onSubmit={submit}>
      ...
      <details className="field full">
        ...
        <div className="form-grid" style={{ marginTop: 12 }}>
          ...
          <label
            className="field"
            title="Silence threshold in dB. More negative = stricter (detects quieter audio as silence)."
          >
          ...
          <label
            className="field"
            title="Minimum contiguous silence duration to consider (seconds)."
          >
          ...
          <div className="field full">
            <p className="muted">
              Uses ffmpeg <code>silencedetect</code> to down-rank clips with more silence. Enable
              only for videos with clear speech/audio tracks.
            </p>
          </div>
          <label className="field full">
            ...
            />
            <p className="muted">
              If set, it’s used for <b>Groq scoring</b> <i>and</i> for slicing real per-clip
              subtitles when <b>Use subtitles</b> is enabled. Generate captions first, then paste
              the output subtitle asset id here.
            </p>
          </label>
          <label className="field">
            ...
            <div className="checkbox-row">
              <label className="checkbox">
                <input
                  type="checkbox"
                  checked={useGroqScoring}
                  onChange={(e) => setUseGroqScoring(e.target.checked)}
                />
      ...
      <label className="field full">
        ...
        <TextArea
          ...
          placeholder="Highlight the funniest moments..."
        />
        {prompt.trim() && !useGroqScoring && (
          <p className="muted">
            Tip: enable Groq scoring (advanced) to use the prompt for segment selection.
          </p>
        )}
        {prompt.trim() && useGroqScoring && (
          <p className="muted">
            Groq scoring uses your prompt + a timed subtitle asset to score segments. Requires{" "}
            <code>GROQ_API_KEY</code> on the worker.
          </p>
        )}
      </label>
      {(useSubtitles || numClips > 6 || maxDuration > 60) && (
        <div className="field full">
          <p className="muted">
            Heads-up: generating many/long clips can take a while. For real per-clip subtitles, set
            a timed subtitle asset id (SRT/VTT) in Advanced selection.
          </p>
        </div>
      )}
      {error && <div className="error-inline">{error}</div>}
...
function AppShell() {
  ...
  const [shortsClips, setShortsClips] = useState<ShortsClip[]>([]);
  ...
  const [shortsStylePresetAll, setShortsStylePresetAll] = useState(
  ...
  const usageMinutesPct = useMemo(() => {
    ...
    return Math.max(
      0,
      Math.min(100, (usageSummary.used_job_minutes / usageSummary.quota_job_minutes) * 100),
    );
  }, [usageSummary]);
  ...
  const filteredProjectAssets = useMemo(() => {
    return projectAssets.filter((asset) =>
      projectAssetKindFilter ? asset.kind === projectAssetKindFilter : true,
    );
  }, [projectAssets, projectAssetKindFilter]);
  const projectAssetKinds = useMemo(
    () => Array.from(new Set(projectAssets.map((asset) => asset.kind))).sort(),
    [projectAssets],
  );
  ...
  const retryJobAndRefresh = async (job: Job) => {
    ...
    try {
      const retried = await apiClient.retryJob(job.id, {
  ...
  const saveBudgetPolicy = async () => {
    ...
    try {
      ...
      setBudgetSoftLimitDraft(
        updated.monthly_soft_limit_cents != null ? String(updated.monthly_soft_limit_cents) : "",
      );
      setBudgetHardLimitDraft(
        updated.monthly_hard_limit_cents != null ? String(updated.monthly_hard_limit_cents) : "",
      );
  ...
  const loadProjectData = async (projectId: string) => {
    ...
    try {
      ...
      setSelectedShareAssetIds((prev) =>
        prev.filter((id) => assetsData.some((asset) => asset.id === id)),
      );
    } catch (err) {
  ...
  const pollJob = (
    job: Job | null,
    onUpdate: (j: Job) => void,
    onAsset?: (a: MediaAsset | null) => void,
  ) => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (
          job.job_type === "shorts" &&
          refreshed.payload &&
          "clip_assets" in (refreshed.payload as any)
        ) {
          ...
          const defaultStylePreset =
            typeof (refreshed.payload as any).style_preset === "string" &&
          ...
          const clips = ((refreshed.payload as any).clip_assets as any[]).map((c, i) => ({
            ...
            style_preset:
              typeof c.style_preset === "string" && c.style_preset.trim()
                ? c.style_preset.trim()
                : defaultStylePreset,
            start: c.start ?? null,
            end: c.end ?? null,
          ...
          setShortsClips((prev) => {
            ...
            return clips.filter(Boolean).map((clip) => {
        ...
        if (
          onAsset &&
          refreshed.output_asset_id &&
          refreshed.output_asset_id !== lastOutputAssetId
        ) {
          try {
        ...
        const progressed =
          refreshed.status !== lastStatus || (refreshed.progress ?? 0) > (lastProgress ?? 0);
  ...
  const renderStyledSubtitlesForClip = async (clip: ShortsClip, previewSeconds?: number) => {
    ...
    if (!clip.subtitle_asset_id) {
      setShortsStyleError(
        "This clip has no subtitle asset. Enable “Use subtitles” when generating shorts.",
      );
      return;
    }
  ...
  const createShareLink = async () => {
    ...
    const assetIds = selectedShareAssetIds.length
      ? selectedShareAssetIds
      : shareSourceAssetId
        ? [shareSourceAssetId]
        : [];
  ...
  const loadOrgInvites = async (roleHint?: string) => {
    const normalizedRole = String(roleHint || orgInfo?.role || "")
      .trim()
      .toLowerCase();
  ...
  const _projectApprovalsFromActivity = (events: ProjectActivityEvent[]): ProjectApproval[] => {
    ...
    for (const event of sorted) {
      ...
      if (
        event.event_type === "project.approval_approved" ||
        event.event_type === "project.approval_rejected"
      ) {
        byId.set(approvalId, {
          ...existing,
  ...
  const revokeScimToken = async (tokenId: string) => {
    ...
    try {
      ...
      setScimTokens((prev) =>
        prev.map((item) =>
          item.id === tokenId ? { ...item, revoked_at: new Date().toISOString() } : item,
        ),
      );
    } catch (err) {
  ...
  const loadProjectCollaboration = async (projectId: string) => {
    ...
    try {
      ...
      setProjectCollabError(
        err instanceof Error ? err.message : "Failed to load project collaboration data",
      );
    } finally {
  ...
  const updateProjectMemberRole = async (userId: string, role: string) => {
    ...
    try {
      ...
      setProjectCollabError(
        err instanceof Error ? err.message : "Failed to update project member role",
      );
    } finally {
      setProjectCollabBusy(false);
    }
      ...
      <main className="content">
        <header className="topbar">
          <div>
            ...
            <p className="lead">
              Kick off captions, translations, styled subtitles, and shorts in one place. Jobs are
              listed below once created through the API.
            </p>
          </div>
            ...
            <Card title="Quick start">
              <ol className="muted">
                ...
                <li>
                  ...
                  <code>make tools-ffmpeg &amp;&amp; make sample-media</code>
                </li>
              ...
              <div className="actions-row">
                ...
                <CopyCommandButton
                  command={`make tools-ffmpeg && make sample-media`}
                  label="Copy sample media command"
                />
              ...
              <>
                ...
                {outputAsset && (
                  <div className="output-card">
                    ...
                    {outputAssetUrl && outputAsset.mime_type?.includes("text") && (
                      <TextPreview url={outputAssetUrl} title="Subtitle preview" />
                    )}
                  </div>
                )}
              </>
        ...
        {active === "shorts" && (
          <section className="grid two-col">
            <Card title="Upload or link video">
              ...
              />
              {safeUploadedPreview && (
                <video className="preview" controls src={safeUploadedPreview} />
              )}
            </Card>
                  ...
                  <div className="progress-bar">
                    <div className="progress-track">
                      <div
                        ...
                        style={{ width: `${Math.round((shortsJob.progress || 0) * 100)}%` }}
                      />
                    </div>
                    ...
                    <div className="actions-row">
                      ...
                      {shortsClips.length > 0 && (
                        <>
                          ...
                          <label
                            className="checkbox"
                            title="Include an audio track line per clip (A) in the EDL export."
                          >
                            <input
                              type="checkbox"
                              checked={timelineIncludeAudio}
                              onChange={(e) => setTimelineIncludeAudio(e.target.checked)}
                            />
                            <span>Audio</span>
                          </label>
                          <label
                            className="checkbox"
                            title="Use unique reel names per clip in the EDL export (helps NLE imports)."
                          >
                            <input
                              type="checkbox"
                              checked={timelinePerClipReel}
                              onChange={(e) => setTimelinePerClipReel(e.target.checked)}
                            />
              ...
              {shortsClips.length > 0 && (
                <div className="actions-row">
                  <label className="field" style={{ margin: 0, minWidth: 220 }}>
                    ...
                    <select
                      className="input"
                      value={shortsStylePresetAll}
                      onChange={(e) => setShortsStylePresetAll(e.target.value)}
                    >
                      {PRESETS.map((p) => (
                        <option key={p.name}>{p.name}</option>
                      ))}
                  ...
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={applyShortsStylePresetToAll}
                  ...
                  <Button
                    ...
                    disabled={
                      shortsStyleAllBusy ||
                      styleClipId !== null ||
                      shortsClips.every((c) => !c.subtitle_asset_id)
                    }
                    title={
                      shortsClips.some((c) => !c.subtitle_asset_id)
                        ? "Some clips have no subtitle asset to render."
                        : undefined
                    }
                  >
                    ...
                    <div className="clip-thumb">
                      {toSafeMediaUrl(clip.thumbnail_uri) ? (
                        <div
                          ...
                          style={{
                            backgroundImage: `url(${toSafeMediaUrl(clip.thumbnail_uri)})`,
                      ...
                      <label className="field">
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setShortsClips((prev) =>
                              prev.map((c) =>
                                c.id === clip.id ? { ...c, style_preset: next } : c,
                              ),
                            );
                          }}
                        >
                        ...
                        <div className="actions-row">
                          ...
                          {clipStyledUrl ? (
                            <Button
                              type="button"
                              variant="ghost"
                              onClick={() => triggerSafeDownload(clipStyledUrl)}
                            >
                              Download styled
                            </Button>
                          ) : (
                          ...
                          {clipSubtitleUrl ? (
                            <Button
                              type="button"
                              variant="ghost"
                              onClick={() => triggerSafeDownload(clipSubtitleUrl)}
                            >
                              Download subs
                            </Button>
                          ) : (
                          ...
                          <Button
                            variant="ghost"
                            onClick={() =>
                              setShortsClips((prev) => prev.filter((c) => c.id !== clip.id))
                            }
                          >
                            Remove
                          </Button>
                          ...
                          <label className="field">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setShortsClips((prev) =>
                                  prev.map((c) =>
                                    c.id === clip.id ? { ...c, start: nextStart } : c,
                                  ),
                                );
                              }}
                            />
                          ...
                          <label className="field">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setShortsClips((prev) =>
                                  prev.map((c) => (c.id === clip.id ? { ...c, end: nextEnd } : c)),
                                );
                              }}
                            />
                          </label>
        ...
        {active === "captions" && (
          <section className="grid two-col">
            ...
            <Card title="Captions & Translate">
              ...
              <CaptionsForm
                onCreated={() => refresh()}
                initialVideoId={uploadedVideoId}
                projectId={selectedProjectId || undefined}
              />
            </Card>
                  ...
                  <div className="snapshot">
                    ...
                    <div>
                      ...
                      <p className="metric-value">
                        {(translateJob.payload as any)?.target_language ?? "n/a"}
                      </p>
                    </div>
                  </div>
                  {toSafeMediaHref(translateOutput?.uri) ? (
                    <div className="actions-row">
                      <a
                        ...
                        href={toSafeMediaHref(translateOutput?.uri)!}
                        download
                      >
                      ...
                      <Button
                        variant="ghost"
                        onClick={() => setSubtitlePreview(toSafeMediaHref(translateOutput?.uri))}
                      >
                        Preview
                      </Button>
                    </div>
          ...
          <section className="grid two-col">
            <Card title="Select assets">
              ...
              <label className="field">
                ...
                <div className="actions-row">
                  ...
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={() => void refreshRecentAssets()}
                    disabled={recentAssetsLoading}
                  >
              ...
              <label className="field">
                <span>Subtitle asset ID</span>
                <Input
                  value={subtitleAssetId}
                  onChange={(e) => setSubtitleAssetId(e.target.value)}
                />
              </label>
              <label className="field">
              ...
              <div className="actions-row">
                <Button
                  ...
                  disabled={
                    !uploadedVideoId ||
                    (captionJob &&
                      !["failed", "cancelled", "completed"].includes(captionJob.status))
                  }
                  onClick={async () => {
                    setCaptionOutput(null);
                    setCaptionJob(null);
                    ...
                    try {
                      ...
                      setAssetError(
                        err instanceof Error ? err.message : "Failed to request captions",
                      );
                    }
                  }}
                >
                  {captionJob && ["running", "queued"].includes(captionJob.status)
                    ? "Generating captions..."
              ...
              {subtitlePreviewUrl && (
                <div className="output-card">
                  <TextPreview
                    url={subtitlePreviewUrl}
                    title={`Subtitle preview ${subtitleFileName ? `(${subtitleFileName})` : ""}`}
                  />
                </div>
              )}
            </Card>
            <Card title="Style editor">
              <p className="muted">
                Tune subtitle styling; if no subtitles are set, we will auto-generate captions
                first.
              </p>
              <StyleEditor
                ...
                <div className="output-card">
                  ...
                  {toSafeMediaHref(styleOutput?.uri) && (
                    <div className="actions-row">
                      <a
                        className="btn btn-primary"
                        href={toSafeMediaHref(styleOutput?.uri)!}
                        download
                      >
                        {styleJob.payload && (styleJob.payload as any).preview_seconds
                          ? "Download preview"
                          : "Download render"}
                      </a>
                    </div>
                  ...
                  {toSafeMediaHref(styleOutput?.uri) &&
                    styleOutput.mime_type?.includes("video") && (
                      <video
                        className="preview"
                        controls
                        src={toSafeMediaHref(styleOutput?.uri)!}
                      />
                    )}
                </div>
              )}
        ...
        {active === "utilities" && (
          <section className="grid two-col">
            <Card title="Subtitle tools">
              <p className="muted">
                Upload or specify subtitle asset to translate; bilingual option available.
              </p>
              <SubtitleToolsForm
                projectId={selectedProjectId || undefined}
                onCreated={(job) => {
              ...
              {subtitleToolsJob && (
                <div className="output-card">
                  ...
                  {["running", "queued"].includes(subtitleToolsJob.status) && (
                    <Spinner label="Polling job status..." />
                  )}
                  <div className="actions-row">
                    {toSafeMediaHref(subtitleToolsOutput?.uri) ? (
                      <a
                        className="btn btn-primary"
                        href={toSafeMediaHref(subtitleToolsOutput?.uri)!}
                        download
                      >
                        Download translated subtitles
                      </a>
            ...
            <Card title="Video / Audio merge">
              <p className="muted">
                Merge audio into a video with optional offset, ducking, and normalization.
              </p>
              <UploadPanel
                projectId={selectedProjectId || undefined}
              ...
              />
              {safeMergeVideoPreview && (
                <video className="preview" controls src={safeMergeVideoPreview} />
              )}
              <AudioUploadPanel
                projectId={selectedProjectId || undefined}
              ...
              {mergeJob && (
                <div className="output-card">
                  ...
                  {["running", "queued"].includes(mergeJob.status) && (
                    <Spinner label="Polling job status..." />
                  )}
                  <div className="actions-row">
                    {toSafeMediaHref(mergeOutput?.uri) ? (
                      <a
                        className="btn btn-primary"
                        href={toSafeMediaHref(mergeOutput?.uri)!}
                        download
                      >
            ...
            <Card title="Jobs">
              <div className="form-grid">
                <label className="field">
                  ...
                  <select
                    ...
                    onChange={(e) => setJobsStatusFilter(e.target.value as JobStatus | "")}
                  >
              ...
              <div className="actions-row">
                <Button
                  type="button"
                  variant="ghost"
                  onClick={() => void loadJobsPage()}
                  disabled={jobsPageLoading}
                >
              ...
              {selectedJob && (
                <>
                  ...
                  <div className="output-card">
                    ...
                    {selectedJob.input_asset_id && (
                      <p className="muted mono">Input: {selectedJob.input_asset_id}</p>
                    )}
                    {selectedJob.output_asset_id && (
                      <p className="muted mono">Output: {selectedJob.output_asset_id}</p>
                    )}
                  ...
                  <div className="actions-row">
                    ...
                    {toSafeMediaHref(outputAsset?.uri) && (
                      <a
                        ...
                        href={toSafeMediaHref(outputAsset?.uri)!}
                        target="_blank"
                        rel="noreferrer"
                      >
                        Open output
                    ...
                    <div className="output-card">
                          ...
                          {toSafeMediaHref(inputAsset?.uri) && (
                            <div className="actions-row">
                              <a
                                ...
                                href={toSafeMediaHref(inputAsset?.uri)!}
                                target="_blank"
                                rel="noreferrer"
                              >
                                Open
                      ...
                      {outputAsset && (
                        <div className="output-card">
                          ...
                          {outputAssetUrl && outputAsset.mime_type?.includes("text") && (
                            <TextPreview url={outputAssetUrl} title="Job output preview" />
                          )}
                        </div>
                      )}
                      {selectedJob.payload && (
        ...
        {active === "usage" && (
          <section className="grid two-col">
            <Card title="Usage summary">
              ...
              {usageSummary && (
                <div className="snapshot">
                  ...
                  <div>
                    ...
                    <p className="metric-value">
                      {usageSummary.output_duration_seconds.toFixed(2)}s
                    </p>
                  </div>
                </div>
              )}
              {usageSummary?.quota_job_minutes != null && (
                <div className="output-card">
                  ...
                  <p className="muted">
                    {(usageSummary.used_job_minutes || 0).toFixed(2)} /{" "}
                    {usageSummary.quota_job_minutes} minutes
                  </p>
                  ...
                  {(usageSummary.overage_job_minutes || 0) > 0 && (
                    <p className="error-inline">
                      Overage preview: {(usageSummary.overage_job_minutes || 0).toFixed(2)} minutes
                    </p>
                  )}
                </div>
              )}
              <div className="output-card">
                ...
                {budgetPolicy ? (
                  <p className="muted">
                    ...
                    {(budgetPolicy.current_month_estimated_cost_cents / 100).toFixed(2)} ·{" "}
                    {budgetProjectedLabel}
                  </p>
                ) : (
                ...
                {budgetPolicy?.projected_status === "hard_limit_exceeded" && (
                  <p className="error-inline">
                    Projected monthly cost is already above hard limit.
                  </p>
                )}
            ...
            <Card title="By job type">
              ...
              {usageSummary && Object.keys(usageSummary.job_type_counts).length === 0 && (
                <p className="muted">No jobs in selected range.</p>
              )}
        ...
        {active === "projects" && (
          <section className="grid two-col">
            <Card title="Projects">
              ...
              <div className="actions-row">
                <Button
                  ...
                  disabled={projectCreateBusy || !newProjectName.trim()}
                >
            ...
            <Card title="Project assets & sharing">
              {!selectedProjectId && (
                <p className="muted">Select a project to inspect jobs/assets.</p>
              )}
              ...
              {selectedProjectId && !projectDataLoading && (
                <>
                  ...
                  <label className="field">
                    ...
                    <select
                      className="input"
                      value={shareSourceAssetId}
                      onChange={(e) => setShareSourceAssetId(e.target.value)}
                    >
                  ...
                  <div className="form-grid">
                    <label className="field">
                      ...
                      <select
                        className="input"
                        value={projectAssetKindFilter}
                        onChange={(e) => setProjectAssetKindFilter(e.target.value)}
                      >
                    ...
                    <div className="output-card">
                      ...
                      <div className="actions-row">
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            setSelectedShareAssetIds(filteredProjectAssets.map((asset) => asset.id))
                          }
                        >
                          Select filtered
                        </Button>
                        <Button
                          type="button"
                          variant="ghost"
                          onClick={() => setSelectedShareAssetIds([])}
                        >
                          Clear selection
                        </Button>
                      </div>
                  ...
                  <div className="actions-row">
                    <Button
                      ...
                      disabled={
                        shareBusy || (selectedShareAssetIds.length === 0 && !shareSourceAssetId)
                      }
                    >
                      {shareBusy
                        ? "Generating..."
                        : `Generate share link${selectedShareAssetIds.length ? "s" : ""}`}
                    </Button>
                  </div>
                  {shareLinks.length > 0 && (
                    <div className="output-card">
            ...
            <Card title="Project collaboration">
              {!selectedProjectId && (
                <p className="muted">
                  Select a project to manage members, comments, and approvals.
                </p>
              )}
                ...
                <>
                  <div className="output-card">
                    ...
                    <div className="form-grid">
                      ...
                      <label className="field">
                        ...
                        <select
                          className="input"
                          value={projectMemberRoleDraft}
                          onChange={(e) => setProjectMemberRoleDraft(e.target.value)}
                        >
                    ...
                    <div className="actions-row">
                      ...
                      <Button
                        type="button"
                        variant="ghost"
                        onClick={() => void loadProjectCollaboration(selectedProjectId)}
                            ...
                            <tr key={member.user_id}>
                              ...
                              <td>
                                <select
                                  ...
                                  value={member.role}
                                  onChange={(e) =>
                                    void updateProjectMemberRole(member.user_id, e.target.value)
                                  }
                                  disabled={projectCollabBusy}
                                >
                                  {ORG_ROLE_OPTIONS.map((role) => (
                              ...
                              <td>
                                <Button
                                  type="button"
                                  variant="ghost"
                                  onClick={() => void removeProjectMember(member.user_id)}
                                  disabled={projectCollabBusy}
                                >
                                  Remove
                                </Button>
                    ...
                    {projectComments.length > 0 ? (
                      <ul className="muted">
                        {projectComments.map((comment) => (
                          <li key={comment.id}>
                            <span className="mono">
                              {comment.author_email || comment.author_user_id}
                            </span>
                            : {comment.body}
                            <Button
                              type="button"
                  ...
                  <div className="output-card">
                    ...
                    <label className="field">
                      ...
                      <Input
                        value={projectApprovalSummaryDraft}
                        onChange={(e) => setProjectApprovalSummaryDraft(e.target.value)}
                        placeholder="Final review for launch assets"
                      />
                    </label>
                    <div className="actions-row">
                      <Button
                        type="button"
                        variant="secondary"
                        onClick={() => void requestProjectApproval()}
                        disabled={projectCollabBusy}
                      >
                                ...
                                {approval.status === "pending" ? (
                                  <div className="actions-row">
                                    <Button
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        void resolveProjectApproval(approval.id, "approve")
                                      }
                                      disabled={projectCollabBusy}
                                    >
                                      Approve
                                    ...
                                    <Button
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        void resolveProjectApproval(approval.id, "reject")
                                      }
                                      disabled={projectCollabBusy}
                                    >
                                      Reject
            ...
            <Card title="Publish automation">
              {!apiClient.accessToken && (
                <p className="muted">Login in Account tab to manage publish providers.</p>
              )}
                ...
                <>
                  <div className="form-grid">
                    <label className="field">
                      ...
                      <select
                        ...
                        onChange={(e) => setPublishProvider(e.target.value as PublishProvider)}
                      >
                    ...
                    <label className="field">
                      ...
                      <Input
                        value={publishConnectCodeDraft}
                        onChange={(e) => setPublishConnectCodeDraft(e.target.value)}
                      />
                    </label>
                    <label className="field">
                      <span>Account ID (optional)</span>
                      <Input
                        value={publishConnectAccountIdDraft}
                        onChange={(e) => setPublishConnectAccountIdDraft(e.target.value)}
                      />
                    </label>
                    <label className="field">
                      <span>Account label (optional)</span>
                      <Input
                        value={publishConnectLabelDraft}
                        onChange={(e) => setPublishConnectLabelDraft(e.target.value)}
                      />
                    </label>
                  </div>
                  <div className="actions-row">
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={() => void connectPublishProvider()}
                    ...
                    <Button
                      type="button"
                      variant="ghost"
                      onClick={() => void loadPublishAutomation()}
                      disabled={publishBusy}
                    >
                      Refresh publish
                    </Button>
                  ...
                  <div className="output-card">
                    ...
                    {publishConnections.length === 0 && (
                      <p className="muted">No active connections for {publishProvider}.</p>
                    )}
                    {publishConnections.length > 0 && (
                      <table className="table">
                        <thead>
                              ...
                              <td>
                                <div className="actions-row">
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    onClick={() => setPublishConnectionIdDraft(connection.id)}
                                  >
                                    Use
                                  </Button>
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    onClick={() => void revokePublishConnection(connection.id)}
                                    disabled={publishBusy}
                                  >
                                    Revoke
                                  </Button>
                  ...
                  <div className="output-card">
                    ...
                    <div className="form-grid">
                      <label className="field">
                        ...
                        <Input
                          value={publishConnectionIdDraft}
                          onChange={(e) => setPublishConnectionIdDraft(e.target.value)}
                        />
                      </label>
                      <label className="field">
                        <span>Asset ID</span>
                        <Input
                          value={publishAssetIdDraft}
                          onChange={(e) => setPublishAssetIdDraft(e.target.value)}
                        />
                      </label>
                      <label className="field">
                        <span>Title</span>
                      ...
                      <label className="field">
                        ...
                        <Input
                          value={publishTagsDraft}
                          onChange={(e) => setPublishTagsDraft(e.target.value)}
                        />
                      </label>
                    </div>
                    <div className="actions-row">
                      <Button
                        ...
                        disabled={
                          publishBusy || !publishConnectionIdDraft || !publishAssetIdDraft.trim()
                        }
                      >
                  ...
                  <div className="output-card">
                    ...
                    {publishJobs.length === 0 && (
                      <p className="muted">No publish jobs for {publishProvider}.</p>
                    )}
                    {publishJobs.length > 0 && (
                      <table className="table">
                        <thead>
                          ...
                          {publishJobs.map((job) => {
                            ...
                            return (
                              <tr key={job.id}>
                                ...
                                <td>
                                  {safePublishUrl ? (
                                    <a href={safePublishUrl} target="_blank" rel="noreferrer">
                                      {safePublishUrl}
                                    </a>
                                  ) : (
                                    "n/a"
                                ...
                                <td>
                                  {job.status === "failed" || job.status === "cancelled" ? (
                                    <Button
                                      type="button"
                                      variant="ghost"
                                      onClick={() => void retryPublishJob(job.id)}
                                      disabled={publishBusy}
                                    >
                                      Retry
                                    </Button>
                ...
                <>
                  ...
                  {inviteResolveToken && (
                    <div className="output-card">
                      ...
                      {!inviteResolveInfo && !inviteResolveError && (
                        <p className="muted">Resolving invite token...</p>
                      )}
                      {inviteResolveError && (
                        <div className="error-inline">{inviteResolveError}</div>
                      )}
                      {inviteResolveInfo && (
                        <>
                          <p className="muted">
                            Invite for <code>{inviteResolveInfo.email}</code> as{" "}
                            <code>{inviteResolveInfo.role}</code>
                          </p>
                          <p className="muted">
                            Expires: {formatTimestamp(inviteResolveInfo.expires_at)}
                          </p>
                          <div className="actions-row">
                            <Button
                              type="button"
                              ...
                              disabled={inviteAcceptBusy || !apiClient.accessToken}
                            >
                          ...
                          {!apiClient.accessToken && (
                            <p className="muted">Log in first, then accept the invite.</p>
                          )}
                        </>
                      )}
                    </div>
                    ...
                    <div className="output-card">
                      ...
                      <div className="actions-row">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          disabled={inviteBusy}
                          onClick={() => void loadOrgInvites(orgInfo.role)}
                        >
                          Refresh invites
                        </Button>
                      </div>
                    ...
                    <tbody>
                      {orgInfo.members.map((member) => (
                        <tr key={member.user_id}>
                          <td>{member.email}</td>
                          <td>
                            {ORG_MANAGER_ROLES.includes(
                              String(orgInfo.role || "").toLowerCase(),
                            ) ? (
                              <select
                                className="input"
                                value={member.role}
                                onChange={(e) =>
                                  void updateMemberRole(member.user_id, e.target.value)
                                }
                                disabled={inviteBusy}
                              >
                                {ORG_ROLE_OPTIONS.map((role) => (
                                  ...
                                  <div className="actions-row">
                                    {safeInviteUrl && (
                                      <Button
                                        type="button"
                                        variant="ghost"
                                        onClick={() => void copyToClipboard(safeInviteUrl)}
                                      >
                                        Copy link
                                      </Button>
                                    )}
                  ...
                  {ORG_MANAGER_ROLES.includes(String(orgInfo.role || "").toLowerCase()) && (
                    <div className="output-card">
                        ...
                        <>
                          <div className="form-grid">
                            <label className="checkbox">
                              <input
                                type="checkbox"
                                checked={ssoEnabledDraft}
                                onChange={(e) => setSsoEnabledDraft(e.target.checked)}
                              />
                            ...
                            <label className="checkbox">
                              <input
                                type="checkbox"
                                checked={ssoJitDraft}
                                onChange={(e) => setSsoJitDraft(e.target.checked)}
                              />
                            ...
                            <label className="checkbox">
                              <input
                                type="checkbox"
                                checked={ssoEmailLinkDraft}
                                onChange={(e) => setSsoEmailLinkDraft(e.target.checked)}
                              />
                            ...
                            <label className="field">
                              ...
                              <Input
                                value={ssoIssuerDraft}
                                onChange={(e) => setSsoIssuerDraft(e.target.value)}
                            ...
                            <label className="field">
                              ...
                              <Input
                                value={ssoClientIdDraft}
                                onChange={(e) => setSsoClientIdDraft(e.target.value)}
                              />
                            </label>
                            <label className="field">
                              <span>Audience (optional)</span>
                              <Input
                                value={ssoAudienceDraft}
                                onChange={(e) => setSsoAudienceDraft(e.target.value)}
                              />
                            </label>
                            <label className="field">
                              ...
                              <select
                                className="input"
                                value={ssoDefaultRoleDraft}
                                onChange={(e) => setSsoDefaultRoleDraft(e.target.value)}
                              >
                                {ORG_ROLE_OPTIONS.map((role) => (
                                  <option key={role} value={role}>
                                    {role}
                          ...
                          <div className="actions-row">
                            <Button
                              type="button"
                              variant="secondary"
                              onClick={() => void saveSsoSecurityConfig()}
                              disabled={ssoSaving}
                            >
                            ...
                            <Button
                              type="button"
                              variant="ghost"
                              onClick={() => void loadSsoSecurityContext()}
                              disabled={ssoSaving}
                            >
                              Refresh SSO
                            </Button>
                            <Button
                              type="button"
                              variant="ghost"
                              onClick={() => void startOktaSsoFlow()}
                              disabled={!ssoEnabledDraft}
                            >
                              Start Okta SSO
                            </Button>
                          </div>
                      ...
                      {ssoConfig?.updated_at && (
                        <p className="muted">
                          Last updated: {formatTimestamp(ssoConfig.updated_at)}
                        </p>
                      )}
                      <div className="output-card">
                        ...
                        {newScimToken && (
                          <div className="output-card">
                            ...
                            <Button
                              type="button"
                              variant="ghost"
                              onClick={() => void copyToClipboard(newScimToken)}
                            >
                              Copy token
                            </Button>
                          </div>
                        ...
                        <div className="actions-row">
                          <Button
                            type="button"
                            variant="secondary"
                            onClick={() => void createScimToken()}
                            disabled={scimCreateBusy}
                          >
                            {scimCreateBusy ? "Working..." : "Create SCIM token"}
                          </Button>
                        </div>
                        {scimTokens.length === 0 && (
                          <p className="muted">No locally-created SCIM tokens in this session.</p>
                        )}
                        {scimTokens.length > 0 && (
                          <table className="table">
                            <thead>
        ...
        {active === "billing" && (
          <section className="grid two-col">
            <Card title="Billing status">
              {!apiClient.accessToken && (
                <p className="muted">Login in Account tab to load billing data.</p>
              )}
              ...
              <div className="actions-row">
                <Button
                  ...
                  disabled={!apiClient.accessToken || billingLoading}
                >
              ...
              {billingSubscription && (
                <div className="snapshot">
                  ...
                  <div>
                    ...
                    <p className="metric-value">
                      {billingSubscription.cancel_at_period_end ? "Yes" : "No"}
                    </p>
                  </div>
                </div>
              )}
              {billingUsage && (
                <div className="output-card">
                  ...
                  <p className="muted">
                    Minutes: {billingUsage.used_job_minutes.toFixed(2)} /{" "}
                    {billingUsage.quota_job_minutes}
                  </p>
                  <p className="muted">
                    Storage: {billingUsage.used_storage_gb.toFixed(2)}GB /{" "}
                    {billingUsage.quota_storage_gb}GB
                  </p>
                  <p className="muted">
                    Estimated overage: ${(billingUsage.estimated_overage_cents / 100).toFixed(2)}
                  </p>
                </div>
              )}
              {billingSeatUsage && (
                <div className="output-card">
                  ...
                  <div className="actions-row">
                    <Button
                      type="button"
                      variant="secondary"
                      disabled={billingLoading || !apiClient.accessToken}
                      onClick={() => void updateSeatLimit()}
                    >
            ...
            <Card title="Plans">
              ...
              <p className="muted">
                Checkout/portal session APIs are available under `/billing/*` for hosted mode
                wiring.
              </p>
            </Card>
          </section>
              ...
              {systemStatus && (
                <>
                  ...
                  <div className="output-card">
                    ...
                    <p className="muted">
                      Ping: {systemStatus.worker.ping_ok ? "ok" : "no response"}
                    </p>
                    ...
                    {systemStatus.worker.error && (
                      <div className="error-inline">{systemStatus.worker.error}</div>
                    )}
                  </div>
                  ...
                  {systemStatus.worker.system_info && (
                    <div className="output-card">
                      ...
                      <pre className="code-block">
                        {JSON.stringify(systemStatus.worker.system_info, null, 2)}
                      </pre>
                    </div>
                  )}
                </>
        ...
        {(active === "shorts" || active === "subtitles") && (
          <section className="grid two-col">
            ...
            <Card title="Notes / prompts">
              <TextArea
                rows={6}
                placeholder="Describe what to prioritize when generating shorts..."
              />
              <div className="actions-row">
