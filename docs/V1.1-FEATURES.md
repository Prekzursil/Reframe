...
> The header here used to read *"DESIGN ONLY — buildable, gate-ready. No implementation
> in this PR"*. That was true when written and false ever since: v1.1.0 shipped and
> §1.2's ASS/BGR colour contract is load-bearing for
> `sidecar/media_studio/features/caption_override.py:17`, so this file is kept, not archived.
> **Date:** 2026-06-28 · **Branch:** `docs/v1.1-feature-spec`
> **Baseline:** merged `main` (#236) + the converged V1 design in
> [`docs/_archive/2026-06/V1-GRILL-DECISIONS.md`](_archive/2026-06/V1-GRILL-DECISIONS.md) (esp. §e/§f/§g/§h) and the
...
CREATE TABLE entity (
  ...
  thumbnail_path TEXT);  -- carries the live Library.thumbnailPath (set_thumbnail, WU-2) — MUST be migrated, not dropped
...
```
- **Migration (hardened to SQLite practice — gate on `user_version`, NOT file existence):** the trigger
  is **`PRAGMA user_version < 1`**, never DB-file existence — opening a SQLite connection CREATES an empty
  file *before* the transaction runs, so a file-existence gate would never re-run after a rolled-back
  migration and would leave an orphaned empty DB authoritative while `library.json` is demoted = silent
  data loss. When `user_version < 1` and `library.json` exists, import each video as a `role='source'`
  entity **carrying ALL fields incl. `thumbnail_path`**. The schema-create + JSON import + **`PRAGMA
  user_version = 1`** stamp all run **inside ONE transaction with the version stamp as the LAST
  statement**, so a crash/rollback anywhere reverts the whole thing and the next open retries from the
  still-authoritative `library.json`. The DB opens in **WAL journal mode**.[^sqlitemig][^sqlitepragma]
  One-way + idempotent (no-op once `user_version >= 1`). On successful migration, **rename `library.json`
  → `library.json.bak`** — a point-in-time backup, NOT a live mirror, so a re-created empty `library.json`
  cannot re-trigger a downgrade. `Library`'s FULL public surface (`add/list/remove/get/set_has_transcript/
  set_thumbnail` + `_normalize`) keeps its signatures (façade over SQLite); the shape regression test
  asserts the **full Video dict (incl. `thumbnailPath`)** survives a migrate→list round-trip — back-compat keystone.
...
## GATE-2 — DataSafety design-review REQUIRED CHANGES (2026-06-28, applied to L1/M3/R0/R1)
The pre-build design-gate BLOCKED on these; all are now folded into the design above + the WU briefs.
- **L1 (BLOCKER — data loss, fixed inline in §3.2):** the `entity` table gained `thumbnail_path TEXT`; the migration carries `thumbnailPath` end-to-end and `set_thumbnail` persists to it; the shape regression test asserts the **full Video dict (incl. `thumbnailPath`)** survives migrate→list. (Old schema silently dropped every poster + broke `set_thumbnail`.)
- **L1 (HIGH — orphaned-empty-DB, fixed inline in §3.2):** migration gates on **`PRAGMA user_version < 1`** (NOT DB-file existence); schema-create + import + `user_version = 1` stamp are ONE transaction with the stamp LAST; on success `library.json` → `library.json.bak` (point-in-time, not a live mirror).
- **M3 (HIGH — fail-closed egress):** a corrupt / missing / half-written persisted `RoutingPolicy` MUST fail **CLOSED to `global:'local'`** (zero egress), never fail-open or crash; `resolve_route` clamps an out-of-enum `mode` to `local`; the policy write is **atomic temp+replace** (mirror `_write_json`). Tests: corrupt-policy→local, unknown-mode→local, unknown-fn→uses global. (Closes Risk #3 silent cloud egress.)
- **R1 (failure-mode split — mirror `reframe.py`/`reframe_claudeshorts.py` contract):** an **EXPLICIT** `reframe_multispeaker` request with WSL/CUDA or models absent **RAISES a typed `OfflineError`**; **AUTO** degrades to single-speaker `claudeshorts` with a loud existing **`"reframe.degraded"`** notice (reuse that notice type, don't invent a channel). Models present-but-fail-to-load = setup-class → propagate `OfflineError`, not a per-clip swallow.
- **R1 (cold-start + OOM):** if the per-segment decision layer fails on the FIRST shot (no last crop), fall back to `claudeshorts` **`select_dominant`** (deterministic), NEVER a silent center crop. An OOM/model-load failure mid-render must **raise a typed error AND clean up the partial output** so no corrupt half-clip is handed to a success path (cross-cuts L2 lineage-on-success).
- **R0 (private-data hygiene):** the golden reference (`Downloads/razvan_gandu`: the RO talk-show + OpusClip's 41 derived clips) is **private/third-party — NEVER committed** and NOT required by the 100%-coverage pure tier. Pure-half metrics run on canned/synthetic path-injected fixtures; the GPU/real-frame tier is opt-in-marked and references the golden set via an external **gitignored** path.
