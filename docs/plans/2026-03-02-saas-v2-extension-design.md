...
- Inputs:
  ...
  - recent check-run contexts for target SHA (`gh api .../check-runs`)
  - policy source (`docs/branch-protection-policy.json` for canonical set)
- Outputs:
  - JSON artifact (`docs/plans/<stamp>-strict23-preflight.json`)
  - markdown summary (`docs/plans/<stamp>-strict23-preflight.md`)
- Status classes:
  - `compliant`: all required canonical contexts observed and green-compatible
  - `non_compliant`: missing or drifted canonical contexts
  - `inconclusive_permissions`: token scope cannot read required policy/check endpoints
...
- Audit behavior change:
  - no "missing protection" claims on permission-denied responses
  - policy mismatch emits exact diff against `docs/branch-protection-policy.json`
- Workflow behavior:
  - create/update finding issue only on true mismatch
  - append run URL and policy delta in issue body
...
1. GitHub API permission gap:
  - classify `inconclusive_permissions`
  - do not emit false non-compliance
2. Missing canonical context:
  - classify `non_compliant`
  - output missing context list and remediation hints
3. Budget policy absent:
  - default to existing plan-based behavior
  - return explicit default object on GET
4. Cost projection unavailable:
  - fail open for transient estimator errors with audit event
  - fail closed only when hard-limit confidence is high and data complete
