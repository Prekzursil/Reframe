...
- **acceptance (falsifiable):** (a) catalog spans ≥3 distinct providers and ranks each model per the 5 tasks; (b) Gemini-free entry flagged `trainsOnInput=True`/`privacyTier=AVOID` for private data; (c) Groq flagged SAFE no-train; (d) every label is dated guidance; (e) docs `docs/providers/MODEL-GUIDE.md` + `docs/providers/SETUP.md` shipped and the "multi-provider, not multi-account" honesty statement present (matches `docs/plans/_archive/provider-hub/CATALOG-SEED.md` ⛔ section).
...
- Multi-PROVIDER rotation only; same-provider extra keys = failover, never advertised ×quota ("N keys ≠ N×quota" honored, SE2 / `docs/plans/_archive/provider-hub/CATALOG-SEED.md`).
...
The single source of truth is **`.github/workflows/quality.yml` gate:3** (verified on `feat/phase8-moment-finding`). `.coverage-thresholds.json` IS the declarative coverage SSOT (added in `e38d5d3a`, PR #235, after this document was written); `quality.yml` gate:3 is its enforcer. The original clause here denied the file existed and was false.
...
4. **Catalog drift (MEDIUM):** free tiers churn; the plan ships DATED curated guidance + a LATER `/models` auto-refresh. Re-verify `docs/plans/_archive/provider-hub/CATALOG-SEED.md` at build time.
