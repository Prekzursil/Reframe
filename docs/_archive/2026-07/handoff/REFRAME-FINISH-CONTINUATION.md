...
> **Status:** ARCHIVED 2026-08-08 — its one unique statement (coverage SSOT = `.coverage-thresholds.json`) was lifted into `docs/INDEX.md` before archiving.
...
## YOUR JOB — Reframe v1.4 finish (``docs/plans/v1.4-remediation.md` (never committed) <!-- ssot-allow: archived record of a path that never existed -->`), command-proven, no self-cert
