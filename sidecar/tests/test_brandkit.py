...
def test_user_caption_style_beats_brand_default() -> None:
    out = brandkit.resolve_brand_defaults({"captionStyle": "neon", "brandCaptionTemplate": "hormozi"})
    assert out["captionStyle"] == "neon"


def test_user_font_beats_brand_default() -> None:
    out = brandkit.resolve_brand_defaults({"captionFontFamily": "Roboto", "brandFontFamily": "Inter"})
    assert out["captionFontFamily"] == "Roboto"


def test_blank_brand_values_are_ignored() -> None:
    out = brandkit.resolve_brand_defaults({"brandCaptionTemplate": "", "brandFontFamily": "   "})
