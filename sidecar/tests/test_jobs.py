...
def test_jobcontext_raise_if_cancelled_raises():
    ev = threading.Event()
    seen = []
    ctx = JobContext(job_id="x", _cancel_event=ev, _emit_progress=lambda *a: seen.append(a))
    ctx.progress(42, "hi")
    assert seen == [("x", 42, "hi")]
...
def _dispatch(method: str, params, ctx: RpcContext):
    req = parse_request({"jsonrpc": "2.0", "id": 1, "method": method, "params": params})
...
def test_record_request_is_first_write_wins_and_keeps_explicit_metadata(registry):
    job = registry.start(lambda ctx: "ok", feature="convert", label="Convert clip", videoId="v7")
    assert job.wait(timeout=5)
