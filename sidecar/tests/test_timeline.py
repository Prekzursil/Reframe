...
class TestTimelinePeaks:
    def test_first_call_decodes_and_returns_shape(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_cache_file_written_with_invalidation_keys(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_cache_hit_skips_ffmpeg_and_is_fast(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_cache_hit_returns_identical_payload(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_source_mtime_change_invalidates_cache(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_source_path_change_invalidates_cache(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_corrupt_cache_is_rebuilt(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_malformed_cache_payload_is_rebuilt(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
        run = FakeRun(pcm_bytes([42]))
        svc = make_service(source, settings, tmp_path, run)
        svc.peaks({"videoId": "vid-1"}, ctx)
        cache_file = tl.peaks_cache_path(tmp_path / "peaks", "vid-1")
    ...
    def test_temp_pcm_file_is_cleaned_up(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
    ...
    def test_custom_bucket_count_is_honored(self, source: Path, settings, tmp_path: Path, ctx: RpcContext):
...
class TestErrors:
    ...
    def test_ffmpeg_failure_surfaces_as_internal_error(self, source, settings, tmp_path, ctx):
    ...
    def test_settings_provider_crash_does_not_break_decode(self, source, tmp_path, ctx, bin_dir, monkeypatch):
