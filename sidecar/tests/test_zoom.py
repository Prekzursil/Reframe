...
def test_beats_deduped_and_sorted() -> None:
    ...
    assert len(beats) == len({round(b, 3) for b in beats})
...
def test_filter_explicit_beats_win_over_cues() -> None:
    ...
    vf = zoom.build_zoom_filter(width=100, height=100, duration_sec=10.0, beats=[5.0], cues=cues)
...
def test_filter_derives_beats_from_cues_when_no_explicit_beats() -> None:
    cues = [cue("a.", 0.0, 1.0, 1), cue("b", 1.0, 2.0, 2)]
    vf = zoom.build_zoom_filter(width=100, height=100, duration_sec=10.0, cues=cues)
...
def test_argv_is_a_list_with_filter_and_progress(monkeypatch) -> None:
    ...
    argv = zoom.build_zoom_argv("in.mp4", "out.mp4", width=1080, height=1920, duration_sec=30.0, cues=[])
...
def test_argv_never_a_shell_string(monkeypatch) -> None:
    monkeypatch.setattr("media_studio.ffmpeg.ffmpeg_path", lambda settings=None: "/bin/ffmpeg")
    argv = zoom.build_zoom_argv("in.mp4", "out.mp4", width=100, height=100, duration_sec=5.0, cues=[])
