...
1c/request placeholder with NO signal it was a stand-in, so the month-to-date
...
turns a run into cents HONESTLY:
  ...
  * :func:`request_cents` / :func:`is_estimated` / :func:`spend_is_estimated` —
    the per-model price, whether a per-model cost is a placeholder, and whether the
...
#: DOCUMENTED PLACEHOLDER rate (cents) for one cloud request when no real per-model
...
def request_cents(model: str | None) -> int:
    """The per-request cost (cents) for ``model``: a real price if known, else the placeholder."""
...
def is_estimated(model: str | None) -> bool:
    """True when ``model``'s cost is the PLACEHOLDER (no real price) — label it an estimate."""
    return not (isinstance(model, str) and model in PRICE_CENTS_PER_REQUEST)


def spend_is_estimated() -> bool:
    """True while the aggregate spend figure is derived from placeholder pricing.

    The month-to-date total is an estimate as long as we hold NO real per-request
    price at all (every model falls back to the placeholder). Once even one real
    price is confirmed, the aggregate is no longer purely placeholder-derived.
    """
