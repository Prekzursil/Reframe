"""Vendored S3FD VGG-backbone face-detection network.

Faithful port of ``Junhua-Liao/Light-ASD``
``model/faceDetector/s3fd/nets.py`` (commit
``ed38c232de5efe0261dbd68627c0ade7cdfe14eb``, MIT). The module list shapes are
byte-for-byte identical so ``sfd_face.pth`` loads without remapping.
...
import torch.nn as nn  # pragma: no cover - heavy s3fd detector
import torch.nn.functional as F  # pragma: no cover - heavy s3fd detector
import torch.nn.init as init  # pragma: no cover - heavy s3fd detector

from .box_utils import Detect, PriorBox  # pragma: no cover - heavy s3fd detector


class L2Norm(nn.Module):  # pragma: no cover - heavy s3fd detector
...
class S3FDNet(nn.Module):  # pragma: no cover - heavy s3fd detector
