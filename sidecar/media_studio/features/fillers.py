...
DEFAULT_SETS: dict[str, dict[str, frozenset]] = {
    ...
    "ro": {
        ...
        "standalone": frozenset(
            {"deci", "gen", "adică", "practic", "păi", "na", "mă rog", "cum să zic", "știi", "stii"}
        ),
    },
}
...
def _lang_sets(lang: str | None, fillers: Mapping[str, Mapping[str, frozenset]]) -> tuple[frozenset, frozenset]:
...
def _is_pause_bounded(words: Sequence[dict[str, Any]], i: int, j: int, gap_sec: float) -> bool:
    ...
    after = (words[j + 1]["start"] - words[j]["end"]) if j + 1 < len(words) else gap_sec
    ...
    while i < n:
        ...
        for length in lengths:
            ...
            if raw_tail.endswith(_SENTENCE_END):
                continue
            if in_standalone and not in_always and not _is_pause_bounded(words, i, j, STANDALONE_GAP_SEC):
                continue
...
def _drop_spans(words: Sequence[dict[str, Any]], drop: Sequence[bool]) -> list[tuple[float, float, int]]:
...
def build_cutlist_with_stats(
    ...
    *,
    ...
    stats = {"fillersRemoved": int(removed_words), "fillerSeconds": round(removed_sec, 3)}
    return keeps, stats
...
def build_cutlist(
    ...
    *,
    ...
    """
    keeps, _stats = build_cutlist_with_stats(words, lang, fillers=fillers, merge_gap_ms=merge_gap_ms, window=window)
    return keeps
...
def remap_cues(cues: Sequence[Mapping[str, Any]], keeps: Sequence[KeepSpan]) -> list[dict[str, Any]]:
