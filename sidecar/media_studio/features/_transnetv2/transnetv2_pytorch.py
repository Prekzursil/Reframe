"""Vendored TransNetV2 shot-transition network (PyTorch).

Faithful copy of ``soCzech/TransNetV2`` ``inference-pytorch/transnetv2_pytorch.py``
(GitHub, **MIT** — see the sibling ``LICENSE``). The re-hosted
``transnetv2.safetensors`` weight (WU I1) is a standard ``convert_weights.py``
export of this exact network at its DEFAULT config, so ``TransNetV2()`` +
``load_state_dict`` matches by construction. ``forward(inputs)`` takes a uint8
``[B, T, 27, 48, 3]`` tensor and returns the single-frame shot-change logits.

HEAVY MODULE — torch is imported at module top (an ``nn.Module`` needs
``torch.nn`` at class-definition time), so it is imported LAZILY only from
:mod:`media_studio.features.scene_transnet_backend` and NEVER during the test
...
import random  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)


class TransNetV2(nn.Module):  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)
...
class StackedDDCNNV2(nn.Module):  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)
...
class DilatedDCNNV2(nn.Module):  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)
...
class Conv3DConfigurable(nn.Module):  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)
...
class FrameSimilarity(nn.Module):  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)
...
class ColorHistograms(nn.Module):  # pragma: no cover - vendored TransNetV2 arch (never imported by the gate)
