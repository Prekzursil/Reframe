"""Vendored ViNet-S video-saliency network (``VideoSaliencyModel``).

Faithful copy of ``ViNet-Saliency/vinet_v2`` ``ViNet_S/ViNet_S_model.py`` with the
cross-file import relativized (``from model_utils import *`` -> ``from
.model_utils import *``). The re-hosted ``vinet-s-saliency.safetensors`` weight
(WU I1; visual-only DHF1K checkpoint) loads into
``VideoSaliencyModel(use_upsample=True, num_hier=3, num_clips=32,
grouped_conv=True, root_grouping=True, ...)`` — the exact config the backend
builds. ICASSP-2025 ViNet++ (arXiv:2502.00397), **CC-BY-NC-SA 4.0**.
...
from .model_utils import *  # pragma: no cover - vendored ViNet-S arch (never imported by the gate)
...
class Interpolate(nn.Module):  # pragma: no cover - vendored ViNet-S arch (never imported by the gate)
...
class BackBoneS3D(nn.Module):  # pragma: no cover - vendored ViNet-S arch (never imported by the gate)
...
class DecoderConvUpGrouped(nn.Module):  # pragma: no cover - vendored ViNet-S arch (never imported by the gate)
...
class DecoderConvUpDepth(nn.Module):  # pragma: no cover - vendored ViNet-S arch (never imported by the gate)
...
class VideoSaliencyModel(nn.Module):  # pragma: no cover - vendored ViNet-S arch (never imported by the gate)
	def __init__(self, 
