...
def _stream_block_reason(stream: dict[str, Any]) -> str | None:
...
def _container_playable(format_name: str, in_path: str) -> bool:
    ...
    if tokens & _PLAYABLE_CONTAINER_TOKENS:
        return True
    return bool("webm" in tokens and Path(in_path).suffix.lower() == ".webm")
...
def build_proxy_argv(in_path: str, out_path: str, settings: dict[str, Any] | None = None) -> list[str]:
...
class MediaCompat:
    ...
    def __init__(
        ...
        *,
        ...
        proxies_dir: str | os.PathLike | None = None,
        probe: ProbeFn | None = None,
        run: RunFn | None = None,
    ) -> None:
        self._resolver = resolver
        self._settings_provider = settings_provider or (lambda: {})
        self._proxies_dir = Path(proxies_dir) if proxies_dir is not None else default_proxies_dir()
    ...
    def _classify_path(self, in_path: str, settings: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    ...
    def _cached_proxy(self, video_id: str, in_path: str) -> Path | None:
    ...
    def _build_derivative(
        ...
        if code != 0:
            partial.unlink(missing_ok=True)
            raise RuntimeError(f"ffmpeg exited with code {code} building the playback proxy for {in_path}")
