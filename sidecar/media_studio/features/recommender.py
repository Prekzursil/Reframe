...
def _download_for(component: str, reason: str) -> DownloadItem:
    """Build one :class:`DownloadItem` for ``component``, enriched from the manifest.

    The manifest is data-only (no network); when it has no entry for the mapped
    asset (not every component registers one yet) we fall back to the asset name
    and a zero size so the proposal is still actionable.
    """
    ...
    if entry is None:
        return DownloadItem(assetName=asset_name, label=asset_name, sizeMb=0, reason=reason)
    return DownloadItem(
...
def _derive_downloads(
    ...
    *,
    ...
    it is not installed, and no detected local server already serves it.
    """
    if offline:
    ...
    for comp in components:
        ...
        if _component_covered_by_servers(name, detected_local):
            continue
        downloads.append(_download_for(name, f"{name} is runnable on this device but its weights are not installed."))
    return downloads
