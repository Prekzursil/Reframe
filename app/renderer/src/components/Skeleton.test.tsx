...
// must never render "a bare LOADING..."; Settings.tsx:157 rendered exactly that.
...
// reduced-motion handling already exist at components/shell.css:634. A second
// copy of that rule is the drift this lane exists to prevent, so the test reads
// the stylesheet and fails if emptyState.css redefines it.
//
...
describe('<Skeleton /> — the shared loading shape', () => {
  ...
  it('becomes an announced status region when the surface labels it', async () => {
    await mount(<Skeleton variant="panel" label="Loading Models & System" />);

    const group = container.querySelector('.skeleton-group');
    expect(group?.getAttribute('role')).toBe('status');
    expect(group?.getAttribute('aria-label')).toBe('Loading Models & System');
...
describe('emptyState.css — rides the ONE .skeleton rule, never a second one', () => {
  ...
  it('does not redefine the .skeleton base (it lives in shell.css:634)', () => {
