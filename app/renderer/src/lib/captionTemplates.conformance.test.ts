...
const VENDOR_TEMPLATES = resolve(REPO_ROOT, 'vendor', 'remotion-captions', 'src', 'templates.ts');
...
const SIDECAR_EMPHASIS = resolve(REPO_ROOT, 'sidecar', 'media_studio', 'features', 'emphasis.py');
...
function sidecarStyleIds(): string[] {
  ...
  // Accept either the modern `list[str]` or the legacy `List[str]` annotation.
  const m = src.match(/STYLES: [Ll]ist\[str\] = \[([\s\S]*?)\]/);
...
describe('caption template three-way mirror conformance (P4 §4 / C3)', () => {
  ...
  it('renderer remotion-engine picker subset == keys(TEMPLATES)', () => {
    const pickerRemotion = CAPTION_STYLES.filter((s) => s.engine === 'remotion').map((s) => s.id);
