...
| --- | --- | --- |
| `preview.spec.ts` | Launches the real app via `playwright._electron.launch`, opens an imported sample video, asserts the preview `<video>` gets an `mstream://` src, reaches `readyState >= 2`, and `currentTime` **advances** after `play()` (real decode). Also asserts no console errors; Library / Workspace / Shorts / SemanticSearch panels mount; `nle.export` yields a real file. | **GUI-VERIFIED** (live app + live sidecar) |
